...
def _import_rnn_fold():
    ...
    try:
        ...
        sys.exit("rnn_fold.py not found.  Place it in the same directory as sparse_prune.py or add it to PYTHONPATH.")
...
def quantization_epsilon(bits: int) -> float:
    ...
    raise ValueError(
        f"Unsupported bit width {bits}. Choose from: 32 (float32), 16 (float16), 8 (int8), 4 (int4), 2 (int2)."
    )
...
def calibrate_min_abs_param(
    ...
    """
    eligible = [(name, t) for name, t in state_dict.items() if isinstance(t, torch.Tensor) and t.ndim == 2]
    ...
    for _, t in eligible:
        ...
        if do_sample:
            n_take = min(flat.numel(), max(1, round(flat.numel() / total_elems * max_sample)))
    ...
    k = max(1, min(all_mags.numel(), round(target_sparsity * all_mags.numel())))
...
def csr_bytes(csr: torch.Tensor) -> int:
    ...
    return csr.values().nbytes + csr.col_indices().nbytes + csr.crow_indices().nbytes
...
def sparsify_model(
    ...
    rnn_fold_band_width: int | None = None,
    ...
    if min_abs_param is not None:
        print("[threshold]  mode             = user-supplied")
        print(f"[threshold]  min_abs_param    = {min_abs_param:.6e}")
    elif calibrate:
        if memory_budget_bytes is not None:
            min_abs_param = calibrate_for_memory_budget(
                state_dict, memory_budget_bytes, min_sparsity=min_sparsity, max_sparse_ratio=max_sparse_ratio
            )
            print("[threshold]  mode             = calibrated, memory-budget-targeted (default)")
            ...
            print("[threshold]  mode             = calibrated, data-driven (default)")
            print(f"[threshold]  target_sparsity  = {target_sparsity:.0%}")
        print(
            f"[threshold]  min_abs_param    = {min_abs_param:.6e}  "
        ...
        print("[threshold]  mode             = training-dynamics formula (legacy, --no-calibrate)")
    ...
    print("[threshold]  vectors (1-D)    = always dense")
    ...
    CW, CS = 56, 20
    hdr = f"  {'Layer':<{CW}} {'Shape':<{CS}} {'Sparsity':>9} {'Dense MB':>9} {'Stored MB':>9} {'Fmt':>7}"
    sep = "-" * len(hdr)
    print(f"\n{hdr}\n{sep}")
    ...
    for name, param in state_dict.items():
        ...
        print(
            f"  {name:<{CW}} {tuple(param.shape)!s:<{CS}}"
            f" {sparsity:>8.1%}"
    ...
    if overall_sp > CATASTROPHE_THRESHOLD:
        ...
        print("!  and may indicate the threshold is miscalibrated for this specific")
        print("!  checkpoint's actual weight scale, not a genuinely well-trained,")
        print("!  highly-sparse model. A high compression ratio here can mean")
        print("!  catastrophic data loss rather than efficient compression.")
        print("!  Consider: lower --target-sparsity, or pass --min-abs explicitly")
        print("!  after inspecting this model's real |weight| distribution.")
        print("!" * 70)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("input", nargs="?", help="Model file (.pt/.safetensors) or shard directory")
    p.add_argument("-o", "--output", default=None, help="Output .pt path  (default: <input>_sparse.pt)")
    p.add_argument(
        "--min-abs",
        type=float,
        default=None,
    ...
    g.add_argument(
        "--lr", type=float, default=1e-5, metavar="F", help="Adam learning rate              (default: 1e-5)"
    )
    g.add_argument(
        "--input-scale", type=float, default=1.0, metavar="F", help="Typical activation magnitude    (default: 1.0)"
    )
    g.add_argument(
        "--days", type=float, default=30.0, metavar="F", help="Training duration in days       (default: 30)"
    )
    g.add_argument(
        "--steps-per-sec", type=float, default=1.0, metavar="F", help="Optimizer steps / second        (default: 1.0)"
    )
    g.add_argument(
        "--bits",
        type=int,
    ...
    d.add_argument(
        ...
        default=0.33,
        metavar="F",
        help="Min fraction of zeros to store sparse (default: 0.33). Below this, keep dense.",
    )
    d.add_argument(
        "--max-sparse-ratio",
        type=float,
        default=0.9,
        metavar="F",
        help="Max csr/dense byte ratio before fallback to dense (default: 0.9).  Vectors (1-D) are always dense.",
    )

    p.add_argument("--show-floor", action="store_true", help="Print synapse floors for all bit widths and exit")

    r = p.add_argument_group("rnn fold (applied after sparsification)")
    r.add_argument("--rnn-fold", action="store_true", help="Fold repeated blocks into stacked CSR matrices")
    r.add_argument(
        "--rnn-fold-skip-outputs", action="store_true", help="Average per-fold-step outputs (pyramidal pooling)"
    )
    r.add_argument(
        "--rnn-fold-min-group",
        type=int,
    ...
    r.add_argument("--rnn-all", action="store_true", help="Extend every eligible layer to a per-call RNN cell")
    return p
...
def main() -> None:
    ...
    if args.show_floor:
        ...
        print(
            f"Training profile: lr={args.lr:.0e}  input={args.input_scale}  "
        ...
        for b, label in rows:
            ...
            note = (
                "  <- most weights ~= 0"
                if b == 2
                else "  <- very high floor"
                if b <= 4
                else "  <- fp16 floor ~ INT8"
                if b == 16
                else ""
            )
    ...
    if args.show_calibration:
        ...
        n_elig = sum(t.numel() for t in state_dict.values() if isinstance(t, torch.Tensor) and t.ndim == 2)
        n_below = sum(
            int((t.detach().float().abs() < thr).sum().item())
            for t in state_dict.values()
    ...
    sparsify_model(
        ...
        memory_budget_bytes=(int(args.memory_budget_mb * 1e6) if args.memory_budget_mb else None),
        learning_rate=args.lr,
        typical_input=args.input_scale,
