...
class CpuBackend(Backend):
    ...
    def pow_backward(self, a, exp: float, grad) -> np.ndarray:
        return exp * np.asarray(a, dtype=np.float32) ** (exp - 1) * np.asarray(grad, dtype=np.float32)
    ...
    def broadcast_to(self, grad, target) -> np.ndarray:
        ...
        shape = (target.rows, target.cols) if isinstance(target, CSR) else np.asarray(target).shape
