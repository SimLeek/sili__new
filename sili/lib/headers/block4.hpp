...
using Block4VecU =
    uint32_t __attribute__((__vector_size__(SILI_BLOCK4_TILE_SIZE * sizeof(uint32_t))));
...
inline Block4VecU block4_vec_quantize_stochastic_fp4(Block4Vec v) {
    ...
    const Block4VecU mag_mid =
        (up_mid_mask & block4_vecu_broadcast(2u)) | (~up_mid_mask & block4_vecu_broadcast(1u));
...
struct Block4Tile {
    ...
    static uint32_t slot_index(uint32_t local_i, uint32_t local_j) {
        return local_j * BLOCK4_TILE + local_i;
    }

    uint8_t& at(uint32_t local_i, uint32_t local_j) { return data[slot_index(local_i, local_j)]; }
    uint8_t at(uint32_t local_i, uint32_t local_j) const {
        return data[slot_index(local_i, local_j)];
    }
...
inline void block4_sparse_set_pos(uint8_t* packed, uint32_t i, uint8_t pos) {
    ...
    if (i % 2 == 0)
        b = uint8_t((b & 0x0Fu) | (pos << 4));
    else
        b = uint8_t((b & 0xF0u) | (pos & 0xFu));
}
...
inline uint8_t block4_sparse_pack(const uint8_t dense[BLOCK4_TILE_SLOTS], uint8_t* packed) {
    ...
    for (std::size_t k = 0; k < nib_bytes; ++k)
        packed[1 + k] = 0; // set_pos does read-modify-write
...
constexpr uint32_t BLOCK4_TILE_SLOTS8_BYTES =
    BLOCK4_TILE_SLOTS * 2; // 32: weight half + importance half
...
struct Block4Tile8 {
    ...
    uint8_t& at_weight(uint32_t local_i, uint32_t local_j) {
        return data[slot_index(local_i, local_j)];
    }
    uint8_t at_weight(uint32_t local_i, uint32_t local_j) const {
        return data[slot_index(local_i, local_j)];
    }
    uint8_t& at_importance(uint32_t local_i, uint32_t local_j) {
        return data[BLOCK4_TILE_SLOTS + slot_index(local_i, local_j)];
    }
    uint8_t at_importance(uint32_t local_i, uint32_t local_j) const {
        return data[BLOCK4_TILE_SLOTS + slot_index(local_i, local_j)];
    }
...
inline uint32_t block4_count_live8(const uint8_t dense[BLOCK4_TILE_SLOTS8_BYTES]) {
    uint32_t n = 0;
    for (uint32_t i = 0; i < BLOCK4_TILE_SLOTS; ++i)
        if (dense[i] != 0 || dense[BLOCK4_TILE_SLOTS + i] != 0)
            ++n;
    return n;
}
...
inline void block4_sparse_unpack8(const uint8_t* packed, uint8_t dense[BLOCK4_TILE_SLOTS8_BYTES]) {
    for (uint32_t i = 0; i < BLOCK4_TILE_SLOTS8_BYTES; ++i)
        dense[i] = 0;
    const uint8_t count = packed[0];
    const std::size_t value_off = 1 + (std::size_t(count) + 1) / 2;
    for (uint8_t i = 0; i < count; ++i) {
...
inline std::size_t block4_stored_tile_len8(bool is_sparse, const uint8_t* tile_bytes) {
    return is_sparse ? block4_sparse_packed_len8(tile_bytes[0])
                     : std::size_t(BLOCK4_TILE_SLOTS8_BYTES);
}

inline void
block4_row_shift(DeltaCSRLayout& L, std::vector<uint8_t>& ibuf,
                 ...
                 std::size_t row, std::size_t target_idx_byte_alloc,
    ...
    if (cur_idx_alloc != target_idx_byte_alloc && row + 1 < L.rows) {
        ...
        if (target_idx_byte_alloc > cur_idx_alloc) {
            ...
            if (new_total > max_indices_bytes)
                throw std::bad_alloc();
        ...
        const std::ptrdiff_t d =
            std::ptrdiff_t(target_idx_byte_alloc) - std::ptrdiff_t(cur_idx_alloc);
    ...
    if (cur_tile_alloc != target_tile_byte_alloc && row + 1 < L.rows) {
        ...
        if (target_tile_byte_alloc > cur_tile_alloc) {
            const std::size_t new_total =
                tile_data.size() + (target_tile_byte_alloc - cur_tile_alloc);
            if (new_total > max_tile_bytes)
                throw std::bad_alloc();
        ...
        const std::ptrdiff_t d =
            std::ptrdiff_t(target_tile_byte_alloc) - std::ptrdiff_t(cur_tile_alloc);
        for (std::size_t r = row + 1; r <= L.rows; ++r)
    ...
    if (cur_elem_alloc != target_elem_alloc && row + 1 < L.rows) {
        ...
        if (move_len > 0)
            std::memmove(tile_is_sparse.data() + new_start, tile_is_sparse.data() + move_src,
                         move_len);
        if (target_elem_alloc < cur_elem_alloc)
...
inline void
block4_ensure_row_headroom(DeltaCSRLayout& L, std::vector<uint8_t>& ibuf,
                           std::vector<std::size_t>& tbyte_start,
                           std::vector<std::size_t>& tbyte_end, std::vector<uint8_t>& tile_data,
                           std::vector<uint8_t>& tile_is_sparse, std::size_t row,
                           std::size_t max_indices_bytes = std::numeric_limits<std::size_t>::max(),
                           std::size_t max_tile_bytes = std::numeric_limits<std::size_t>::max()) {
    const std::size_t target_idx = L.row_alloc_bytes(row) + uleb128_max_bytes<uint32_t>();
    const std::size_t target_tile =
        (tbyte_start[row + 1] - tbyte_start[row]) + block4_sparse_packed_len(0);
    ...
    if (row + 1 < L.rows)
        ...
        block4_grow_last_row(L, ibuf, tbyte_start, tbyte_end, tile_data, tile_is_sparse, target_idx,
                             target_tile, target_elem, max_indices_bytes, max_tile_bytes);
}
...
block4_row_insert_tile(DeltaCSRLayout& L, std::vector<uint8_t>& ibuf,
    ...
    for (std::size_t e = 0; e < n; ++e) {
        ...
        if (col == new_col)
            return false; // duplicate, skip (tile already exists)
        if (col > new_col) {
        ...
        const std::size_t tlen =
            tile_len_fn(tile_is_sparse[elem_pos], tile_data.data() + tbyte_pos);
        prev_col = col;
        byte_pos += dlen;
        elem_pos++;
        tbyte_pos += tlen;
...

    {
        ...
        if (shift_len > 0)
            std::memmove(tile_data.data() + shift_from + new_tile_len,
                         tile_data.data() + shift_from, shift_len);
    ...
    for (std::size_t e = 0; e < n; ++e) {
        ...
        if (col == target_col) {
            ...
            if (e + 1 < n) {
                ...
                if (shift_len > 0)
                    std::memmove(ibuf.data() + byte_pos + merged_len, ibuf.data() + shift_from,
                                 shift_len);
...

            {
                ...
                const std::size_t shift_len = tbyte_end[row] - shift_from;
                if (shift_len > 0)
                    std::memmove(tile_data.data() + tbyte_pos, tile_data.data() + shift_from,
                                 shift_len);
                tbyte_end[row] -= tlen;
            }
...
inline bool block4_row_grow_needs_realloc(const std::vector<uint8_t>& tile_data,
                                          const std::vector<std::size_t>& tbyte_start,
                                          const std::vector<std::size_t>& tbyte_end,
                                          std::size_t row, std::size_t old_len,
                                          std::size_t new_len) {
    if (new_len <= old_len)
        return false;
    ...
    if (delta <= headroom)
        return false; // pure in-row shift, tile_data.resize() never called
    ...
    if (delta > 0) {
        ...
        if (std::size_t(delta) > headroom) {
            ...
            if (row + 1 < L.rows) {
                ...
                const std::size_t move_len = tbyte_start[L.rows] - move_src;
                tile_data.resize(new_total);
                std::memmove(tile_data.data() + move_src + shortfall, tile_data.data() + move_src,
                             move_len);
                for (std::size_t r = row + 1; r <= L.rows; ++r)
                    tbyte_start[r] += shortfall;
                for (std::size_t r = row + 1; r < L.rows; ++r)
                    tbyte_end[r] += shortfall;
            } else {
                tile_data.resize(new_total);
...
class Block4TileHandle {
  ...
  public:
    ...
    Block4TileHandle(Block4TileHandle&& other) noexcept {
        ...
        br_ = other.br_;
        bc_ = other.bc_;
        byte_pos_ = other.byte_pos_;
        std::memcpy(scratch_, other.scratch_, sizeof(scratch_));
        was_sparse_ = other.was_sparse_;
        dirty_ = other.dirty_;
        valid_ = other.valid_;
        other.valid_ = false;
        other.dirty_ = false; // moved-from: destructor becomes a no-op
    }
...
struct Block4Store {
    ...
    std::size_t raw_find(uint32_t br, uint32_t bc, std::size_t* out_elem_pos = nullptr) const {
        ...
        for (std::size_t e = 0; e < n; ++e, ++elem_pos) {
            ...
            if (col == bc) {
                if (out_elem_pos)
                    *out_elem_pos = elem_pos;
                return byte_pos;
            }
            if (col > bc)
                break; // sorted ascending -- can't appear later
    ...
    Block4TileHandle get_or_create(uint32_t br, uint32_t bc) {
        if (raw_find(br, bc) == std::numeric_limits<std::size_t>::max()) {
            if (br >= block_layout.rows)
                throw std::out_of_range("Block4Store::get_or_create: block_row out of range -- "
            ...
            if (!block4_row_insert_tile(block_layout, indices_buf, tile_byte_start, tile_byte_end,
                ...
                block4_ensure_row_headroom(block_layout, indices_buf, tile_byte_start,
                                           tile_byte_end, tile_data, tile_is_sparse, br,
                                           max_indices_bytes, max_tile_bytes);
                const bool ok =
                    block4_row_insert_tile(block_layout, indices_buf, tile_byte_start,
                                           tile_byte_end, tile_data, tile_is_sparse, br, bc);
                (void)ok; // block4_ensure_row_headroom grows by exactly enough for one more tile --
                          // this must succeed
            }
        }
    ...
    void erase(uint32_t br, uint32_t bc) {
        if (br >= block_layout.rows)
            return;
        block4_row_remove_tile(block_layout, indices_buf, tile_byte_start, tile_byte_end, tile_data,
                               tile_is_sparse, br, bc);
    }
    ...
    void maybe_compress(uint32_t br, uint32_t bc) {
        ...
        if (byte_pos == std::numeric_limits<std::size_t>::max() || tile_is_sparse[elem_pos])
            return;
        ...
        block4_resize_tile_in_row(block_layout, tile_byte_start, tile_byte_end, tile_data, br,
                                  byte_pos, BLOCK4_TILE_SLOTS, packed, new_len, max_tile_bytes);
    ...
    void commit_dirty_sparse_tile(uint32_t br, uint32_t bc,
                                  const uint8_t scratch[BLOCK4_TILE_SLOTS]) {
        ...
        if (fresh_byte_pos == std::numeric_limits<std::size_t>::max())
            return; // was already erased
        ...
        if (!block4_row_grow_needs_realloc(tile_data, tile_byte_start, tile_byte_end, br, cur_len,
                                           new_len)) {
            try {
                block4_resize_tile_in_row(block_layout, tile_byte_start, tile_byte_end, tile_data,
                                          br, fresh_byte_pos, cur_len, packed, new_len,
                                          max_tile_bytes);
                tile_is_sparse[elem_pos] = now_sparse ? 1 : 0;
            } catch (const std::bad_alloc&) {
                std::atomic_ref<std::uint64_t>(dropped_growth_events)
        ...
        try {
            block4_resize_tile_in_row(block_layout, tile_byte_start, tile_byte_end, tile_data, br,
                                      fresh_byte_pos, cur_len, packed, new_len, max_tile_bytes);
            tile_is_sparse[elem_pos] = now_sparse ? 1 : 0;
        } catch (const std::bad_alloc&) {
            std::atomic_ref<std::uint64_t>(dropped_growth_events)
                .fetch_add(1, std::memory_order_relaxed);
    ...
    void commit_dirty_tile_in_workspace(RowWorkspace& ws, std::size_t e, std::size_t local_byte_pos,
        ...
        const std::size_t cur_len =
            block4_stored_tile_len(ws.is_sparse[e], &ws.bytes[local_byte_pos]);
        block4_resize_tile_in_row(ws.local_L, ws.local_tbyte_start, ws.local_tbyte_end, ws.bytes, 0,
                                  local_byte_pos, cur_len, packed, new_len,
    ...
    static void unpack_workspace_tile(const RowWorkspace& ws, std::size_t e,
        ...
        if (ws.is_sparse[e])
            ...
            std::memcpy(scratch, &ws.bytes[local_byte_pos], BLOCK4_TILE_SLOTS);
    }

    // Writes the workspace back into the shared store
    template <typename TrueImportanceFn>
    std::size_t merge_row_workspace(std::size_t br, RowWorkspace& ws,
                                    TrueImportanceFn&& true_importance) {
        ...
        while (ws.local_tbyte_end[0] > global_headroom) {
                ...
                for (uint32_t li = 0; li < BLOCK4_TILE; ++li) {
                    for (uint32_t lj = 0; lj < BLOCK4_TILE; ++lj) {
                        ...
                        const std::size_t col =
                            static_cast<std::size_t>(ws.bc[e]) * BLOCK4_TILE + lj;
                        const double abs_imp =
                            std::abs(static_cast<double>(true_importance(row, col, imp_code)));
            ...
            if (best_abs_imp < 0.0)
                break; // nothing live left to evict -- see the
        ...
        for (std::size_t e = 0; e < ws.row_nnz; ++e) {
            if (fit_pos >= ws.bytes.size())
                break;
            const std::size_t tlen = block4_stored_tile_len(ws.is_sparse[e], &ws.bytes[fit_pos]);
            if (fit_pos + tlen > global_headroom)
                break;
            fit_pos += tlen;
            ++fit_tiles;
        }
        if (fit_tiles < ws.row_nnz) {
            std::atomic_ref<std::uint64_t>(row_merge_overflow_events)
                .fetch_add(1, std::memory_order_relaxed);
    ...
    std::size_t total_tile_used_bytes() const {
        ...
        for (std::size_t r = 0; r < block_layout.rows; ++r)
            n += tile_byte_end[r] - tile_byte_start[r];
        return n;
    }
...
inline uint32_t Block4TileHandle::count_live() const {
    return was_sparse_ ? block4_count_live(scratch_)
                       : block4_count_live(&store_->tile_data[byte_pos_]);
}
...
class Block4TileHandle8 {
  ...
  public:
    ...
    Block4TileHandle8(Block4Store8& store, uint32_t br, uint32_t bc, std::size_t elem_pos,
                      std::size_t byte_pos);
    ~Block4TileHandle8();
...
struct Block4Store8 {
    ...
    Block4TileHandle8 at_index(uint32_t br, uint32_t bc, std::size_t elem_pos,
                               std::size_t byte_pos) {
    ...
    Block4TileHandle8 get_or_create(uint32_t br, uint32_t bc) {
        if (raw_find(br, bc) == std::numeric_limits<std::size_t>::max()) {
            if (br >= block_layout.rows)
                throw std::out_of_range("Block4Store8::get_or_create: block_row out of range -- "
                                        "was Block4Store8::init(n_in, n_out) called?");
            if (!block4_row_insert_tile(block_layout, indices_buf, tile_byte_start, tile_byte_end,
                                        tile_data, tile_is_sparse, br, bc,
                                        block4_stored_tile_len8)) {
                block4_ensure_row_headroom(block_layout, indices_buf, tile_byte_start,
                                           tile_byte_end, tile_data, tile_is_sparse, br,
                                           max_indices_bytes, max_tile_bytes);
                const bool ok = block4_row_insert_tile(block_layout, indices_buf, tile_byte_start,
                                                       tile_byte_end, tile_data, tile_is_sparse, br,
                                                       bc, block4_stored_tile_len8);
                (void)ok;
            }
        }
    ...
    void erase(uint32_t br, uint32_t bc) {
        if (br >= block_layout.rows)
            return;
        block4_row_remove_tile(block_layout, indices_buf, tile_byte_start, tile_byte_end, tile_data,
                               tile_is_sparse, br, bc, block4_stored_tile_len8);
    }
    ...
    void maybe_compress(uint32_t br, uint32_t bc) {
        ...
        block4_resize_tile_in_row(block_layout, tile_byte_start, tile_byte_end, tile_data, br,
                                  byte_pos, BLOCK4_TILE_SLOTS8_BYTES, packed, new_len,
                                  max_tile_bytes);
        tile_is_sparse[elem_pos] = 1;
    }
    ...
    void commit_dirty_sparse_tile(uint32_t br, uint32_t bc,
                                  const uint8_t scratch[BLOCK4_TILE_SLOTS8_BYTES]) {
        std::size_t elem_pos = 0;
        const std::size_t fresh_byte_pos = raw_find(br, bc, &elem_pos);
        if (fresh_byte_pos == std::numeric_limits<std::size_t>::max())
            return;
    ...
    void commit_dirty_tile_in_workspace(RowWorkspace& ws, std::size_t e, std::size_t local_byte_pos,
        ...
        const std::size_t cur_len =
            block4_stored_tile_len8(ws.is_sparse[e], &ws.bytes[local_byte_pos]);
        block4_resize_tile_in_row(ws.local_L, ws.local_tbyte_start, ws.local_tbyte_end, ws.bytes, 0,
                                  local_byte_pos, cur_len, packed, new_len,
                                  std::numeric_limits<std::size_t>::max());
        ws.is_sparse[e] = now_sparse ? 1 : 0;
    ...
    static void unpack_workspace_tile(const RowWorkspace& ws, std::size_t e,
        ...
        if (ws.is_sparse[e])
            ...
            std::memcpy(scratch, &ws.bytes[local_byte_pos], BLOCK4_TILE_SLOTS8_BYTES);
    }
...
inline Block4TileHandle8::Block4TileHandle8(Block4Store8& store, uint32_t br, uint32_t bc)
    ...
    if (bp == std::numeric_limits<std::size_t>::max()) {
        valid_ = false;
        return;
    }
    valid_ = true;
    ...
    if (was_sparse_)
        block4_sparse_unpack8(&store_->tile_data[byte_pos_], scratch_);
}

inline Block4TileHandle8::Block4TileHandle8(Block4Store8& store, uint32_t br, uint32_t bc,
                                            std::size_t elem_pos, std::size_t byte_pos)
...
inline uint32_t Block4TileHandle8::count_live() const {
    return was_sparse_ ? block4_count_live8(scratch_)
                       : block4_count_live8(&store_->tile_data[byte_pos_]);
}
...
inline Block4TileHandle::Block4TileHandle(Block4Store& store, uint32_t br, uint32_t bc)
    ...
    was_sparse_ = bool(store_->tile_is_sparse[elem_pos]);
    if (was_sparse_)
        block4_sparse_unpack(&store_->tile_data[byte_pos_], scratch_);
}
...
struct Block4Tile32 {
    uint8_t data[BLOCK4_TILE_SLOTS32_BYTES] = {0};

    static uint32_t slot_index(uint32_t local_i, uint32_t local_j) {
        return local_j * BLOCK4_TILE + local_i;
    }

    float get_weight(uint32_t local_i, uint32_t local_j) const {
        float v;
        std::memcpy(&v, data + sizeof(float) * slot_index(local_i, local_j), sizeof(v));
        return v;
    }
    void set_weight(uint32_t local_i, uint32_t local_j, float v) {
        std::memcpy(data + sizeof(float) * slot_index(local_i, local_j), &v, sizeof(v));
    }
    float get_importance(uint32_t local_i, uint32_t local_j) const {
        float v;
        std::memcpy(&v, data + sizeof(float) * (BLOCK4_TILE_SLOTS + slot_index(local_i, local_j)),
                    sizeof(v));
        return v;
    }
    void set_importance(uint32_t local_i, uint32_t local_j, float v) {
        std::memcpy(data + sizeof(float) * (BLOCK4_TILE_SLOTS + slot_index(local_i, local_j)), &v,
                    sizeof(v));
    }

    uint32_t count_live() const; // defined after block4_count_live32 below
};

// A slot is live iff EITHER its weight or importance is nonzero -- same
// "not both zero" convention as block4_count_live8.
inline uint32_t block4_count_live32(const uint8_t dense[BLOCK4_TILE_SLOTS32_BYTES]) {
    uint32_t n = 0;
    for (uint32_t i = 0; i < BLOCK4_TILE_SLOTS; ++i) {
        float w, imp;
        std::memcpy(&w, dense + sizeof(float) * i, sizeof(w));
        std::memcpy(&imp, dense + sizeof(float) * (BLOCK4_TILE_SLOTS + i), sizeof(imp));
        if (w != 0.0f || imp != 0.0f)
            ++n;
    }
    return n;
...
// (false positive, same reason as Block4Tile8::count_live -- cppcheck
// doesn't attribute the implicit array-to-pointer decay of `data` as a use
// of the instance's own member)
// cppcheck-suppress functionStatic
inline uint32_t Block4Tile32::count_live() const {
    return block4_count_live32(data);
}

// Sparse-packed FP32 tile encoding -- same position-nibble scheme as
// FP4/FP8's block4_sparse_pack(8) (4-bit slot position, 2 packed per byte),
// but 2 full float32 VALUES per live entry (weight, then importance -- two
// contiguous runs, same halves convention as the dense layout above)
// instead of 1-2 bytes.
//
// Budget: count*68+8<=1024 (128 bytes = 1024 bits, matching Block4Tile32's
// own dense size) -- 68 bits/entry (4 position + 32 weight + 32 importance)
// vs FP8's 20 (4 position + 16 value) and FP4's 12 (4 position + 8 value).
// Max count: 14 (14*68+8=960<=1024; 15*68+8=1028>1024) -- a real, correctly
// re-derived value, not a copy of FP4/FP8's constants.
#ifndef SILI_BLOCK4_SPARSE_MAX_COUNT32
#define SILI_BLOCK4_SPARSE_MAX_COUNT32 14
#endif
static_assert(SILI_BLOCK4_SPARSE_MAX_COUNT32 * 68 + 8 <= 1024,
              "sparse FP32 tile encoding must fit in the same 1024 bits as the dense one");
constexpr uint32_t BLOCK4_SPARSE_MAX_COUNT32 = SILI_BLOCK4_SPARSE_MAX_COUNT32;

inline std::size_t block4_sparse_packed_len32(uint8_t count) {
    return std::size_t(1) + (std::size_t(count) + 1) / 2 + std::size_t(count) * 2 * sizeof(float);
}

// Position nibble packing is byte-identical to FP4/FP8's -- reuses
// block4_sparse_get_pos/block4_sparse_set_pos directly (position encoding
// has nothing to do with value width).

inline void block4_sparse_unpack32(const uint8_t* packed,
                                   uint8_t dense[BLOCK4_TILE_SLOTS32_BYTES]) {
    for (uint32_t i = 0; i < BLOCK4_TILE_SLOTS32_BYTES; ++i)
        dense[i] = 0;
    const uint8_t count = packed[0];
    const std::size_t value_off = 1 + (std::size_t(count) + 1) / 2;
    for (uint8_t i = 0; i < count; ++i) {
        const uint8_t pos = block4_sparse_get_pos(packed, i);
        std::memcpy(dense + sizeof(float) * pos, packed + value_off + sizeof(float) * i,
                    sizeof(float)); // weight
        std::memcpy(dense + sizeof(float) * (BLOCK4_TILE_SLOTS + pos),
                    packed + value_off + sizeof(float) * (std::size_t(count) + i),
                    sizeof(float)); // importance
    }
}

inline uint8_t block4_sparse_pack32(const uint8_t dense[BLOCK4_TILE_SLOTS32_BYTES],
                                    uint8_t* packed) {
    uint8_t count = 0;
    for (uint32_t i = 0; i < BLOCK4_TILE_SLOTS; ++i) {
        float w, imp;
    ...
    for (uint32_t i = 0; i < BLOCK4_TILE_SLOTS; ++i) {
        ...
        if (w == 0.0f && imp == 0.0f)
            continue;
        block4_sparse_set_pos(packed, idx, uint8_t(i));
        std::memcpy(packed + value_off + sizeof(float) * idx, &w, sizeof(w));
        std::memcpy(packed + value_off + sizeof(float) * (std::size_t(count) + idx), &imp,
                    sizeof(imp));
        ++idx;
    }
    return count;
...
inline std::size_t block4_stored_tile_len32(bool is_sparse, const uint8_t* tile_bytes) {
    return is_sparse ? block4_sparse_packed_len32(tile_bytes[0])
                     : std::size_t(BLOCK4_TILE_SLOTS32_BYTES);
}

struct Block4Store32;

class Block4TileHandle32 {
    Block4Store32* store_ = nullptr;
    uint32_t br_ = 0, bc_ = 0;
    std::size_t byte_pos_ = 0;
    uint8_t scratch_[BLOCK4_TILE_SLOTS32_BYTES] = {0};
    bool was_sparse_ = false;
    bool dirty_ = false;
    bool valid_ = false;

  public:
    Block4TileHandle32() = default;
    Block4TileHandle32(Block4Store32& store, uint32_t br, uint32_t bc);
    Block4TileHandle32(Block4Store32& store, uint32_t br, uint32_t bc, std::size_t elem_pos,
                       std::size_t byte_pos);
    ~Block4TileHandle32();

    Block4TileHandle32(Block4TileHandle32&& other) noexcept {
        store_ = other.store_;
        br_ = other.br_;
        bc_ = other.bc_;
        byte_pos_ = other.byte_pos_;
    ...
    Block4TileHandle32& operator=(Block4TileHandle32&& other) noexcept {
        if (this == &other)
            return *this;
        this->~Block4TileHandle32();
        store_ = other.store_;
        br_ = other.br_;
        bc_ = other.bc_;
        byte_pos_ = other.byte_pos_;
    ...
    Block4TileHandle32(const Block4TileHandle32&) = delete;
    Block4TileHandle32& operator=(const Block4TileHandle32&) = delete;

    explicit operator bool() const { return valid_; }

    float get_weight(uint32_t li, uint32_t lj) const;
    void set_weight(uint32_t li, uint32_t lj, float v);
    float get_importance(uint32_t li, uint32_t lj) const;
    void set_importance(uint32_t li, uint32_t lj, float v);
    uint32_t count_live() const;

    // Raw pointer to this tile's 128 bytes (scratch_ if sparse, tile_data+
    // byte_pos_ if dense) -- weight half [0..63], importance half
    // [64..127], matching Block4Tile32's own layout. The mutable overload
    // marks the handle dirty (used by the SIMD forward/backward hot loop
    // in linear_disldo.hpp, which block4_vec_store's directly into this
    // buffer instead of going through set_weight/set_importance one lane
    // at a time).
    const uint8_t* raw_data() const;
    uint8_t* raw_data_mutable();
};
...
struct Block4Store32 {
    ...
    // Default BLOCK4_SPARSE_MAX_COUNT32 (14, the exact 14*68+8=960<=1024
    // arithmetic -- see block4_sparse_packed_len32's own comment).
    uint32_t switch_point = BLOCK4_SPARSE_MAX_COUNT32;
    ...
    void set_limits(std::size_t indices_limit_bytes, std::size_t tile_limit_bytes) {
        max_indices_bytes = indices_limit_bytes;
        max_tile_bytes = tile_limit_bytes;
        // See Block4Store::set_limits's identical comment -- no try/catch
        // here (previously silently swallowed), same reason.
        if (tile_limit_bytes != std::numeric_limits<std::size_t>::max()) {
            tile_data.reserve(tile_limit_bytes);
        }
    }
    ...
    // See Block4Store::row_merge_overflow_events' own comment -- identical
    // mechanism, mirrored here for FP32.
    std::uint64_t row_merge_overflow_events = 0;
    std::uint64_t row_merge_overflow_bytes_dropped = 0;
    ...
    std::size_t tile_len_at(std::size_t elem_pos, std::size_t byte_pos) const {
        return block4_stored_tile_len32(tile_is_sparse[elem_pos], &tile_data[byte_pos]);
    }
    ...
    Block4TileHandle32 find(uint32_t br, uint32_t bc) { return Block4TileHandle32(*this, br, bc); }
    Block4TileHandle32 find(uint32_t br, uint32_t bc) const {
        return const_cast<Block4Store32*>(this)->find(br, bc);
    }

    Block4TileHandle32 at_index(uint32_t br, uint32_t bc, std::size_t elem_pos,
                                std::size_t byte_pos) {
        return Block4TileHandle32(*this, br, bc, elem_pos, byte_pos);
    }
    Block4TileHandle32 at_index(uint32_t br, uint32_t bc, std::size_t elem_pos,
                                std::size_t byte_pos) const {
        return const_cast<Block4Store32*>(this)->at_index(br, bc, elem_pos, byte_pos);
    }

    Block4TileHandle32 get_or_create(uint32_t br, uint32_t bc) {
        if (raw_find(br, bc) == std::numeric_limits<std::size_t>::max()) {
            if (br >= block_layout.rows)
                throw std::out_of_range("Block4Store32::get_or_create: block_row out of range -- "
                                        "was Block4Store32::init(n_in, n_out) called?");
            if (!block4_row_insert_tile(block_layout, indices_buf, tile_byte_start, tile_byte_end,
                                        tile_data, tile_is_sparse, br, bc,
                                        block4_stored_tile_len32)) {
                block4_ensure_row_headroom(block_layout, indices_buf, tile_byte_start,
                                           tile_byte_end, tile_data, tile_is_sparse, br,
                                           max_indices_bytes, max_tile_bytes);
                const bool ok = block4_row_insert_tile(block_layout, indices_buf, tile_byte_start,
                                                       tile_byte_end, tile_data, tile_is_sparse, br,
                                                       bc, block4_stored_tile_len32);
                (void)ok;
            }
        }
        return Block4TileHandle32(*this, br, bc);
    }
    ...
    void erase(uint32_t br, uint32_t bc) {
        if (br >= block_layout.rows)
            return;
        block4_row_remove_tile(block_layout, indices_buf, tile_byte_start, tile_byte_end, tile_data,
                               tile_is_sparse, br, bc, block4_stored_tile_len32);
    }
    ...
    void maybe_compress(uint32_t br, uint32_t bc) {
        ...
        if (byte_pos == std::numeric_limits<std::size_t>::max() || tile_is_sparse[elem_pos])
            return;
        const uint32_t n = block4_count_live32(&tile_data[byte_pos]);
        if (n > switch_point)
            return;
        uint8_t packed[BLOCK4_TILE_SLOTS32_BYTES];
        block4_sparse_pack32(&tile_data[byte_pos], packed);
        const std::size_t new_len = block4_sparse_packed_len32(uint8_t(n));
        block4_resize_tile_in_row(block_layout, tile_byte_start, tile_byte_end, tile_data, br,
                                  byte_pos, BLOCK4_TILE_SLOTS32_BYTES, packed, new_len,
                                  max_tile_bytes);
        tile_is_sparse[elem_pos] = 1;
    }
    ...
    void commit_dirty_sparse_tile(uint32_t br, uint32_t bc,
                                  const uint8_t scratch[BLOCK4_TILE_SLOTS32_BYTES]) {
        std::size_t elem_pos = 0;
        const std::size_t fresh_byte_pos = raw_find(br, bc, &elem_pos);
        if (fresh_byte_pos == std::numeric_limits<std::size_t>::max())
            return;

        const uint32_t n = block4_count_live32(scratch);
        uint8_t packed[BLOCK4_TILE_SLOTS32_BYTES];
        std::size_t new_len;
        bool now_sparse;
        if (n <= switch_point) {
            block4_sparse_pack32(scratch, packed);
            new_len = block4_sparse_packed_len32(uint8_t(n));
            now_sparse = true;
        } else {
            std::memcpy(packed, scratch, BLOCK4_TILE_SLOTS32_BYTES);
            new_len = BLOCK4_TILE_SLOTS32_BYTES;
            now_sparse = false;
        }
        const std::size_t cur_len = tile_len_at(elem_pos, fresh_byte_pos);
        // See Block4Store::commit_dirty_sparse_tile's identical fast/
        // fallback-path comment -- same fix, same reason, FP32's own
        // tile_data.
        if (!block4_row_grow_needs_realloc(tile_data, tile_byte_start, tile_byte_end, br, cur_len,
                                           new_len)) {
            try {
    ...
    static std::size_t workspace_tile_byte_pos(const RowWorkspace& ws, std::size_t e) {
        std::size_t pos = 0;
        for (std::size_t i = 0; i < e; ++i)
            pos += block4_stored_tile_len32(ws.is_sparse[i], &ws.bytes[pos]);
        return pos;
    }

    void commit_dirty_tile_in_workspace(RowWorkspace& ws, std::size_t e, std::size_t local_byte_pos,
                                        const uint8_t scratch[BLOCK4_TILE_SLOTS32_BYTES]) const {
        const uint32_t n = block4_count_live32(scratch);
        uint8_t packed[BLOCK4_TILE_SLOTS32_BYTES];
        std::size_t new_len;
        bool now_sparse;
        ...
        const std::size_t cur_len =
            block4_stored_tile_len32(ws.is_sparse[e], &ws.bytes[local_byte_pos]);
        block4_resize_tile_in_row(ws.local_L, ws.local_tbyte_start, ws.local_tbyte_end, ws.bytes, 0,
                                  local_byte_pos, cur_len, packed, new_len,
                                  std::numeric_limits<std::size_t>::max());
        ws.is_sparse[e] = now_sparse ? 1 : 0;
    ...
    static void unpack_workspace_tile(const RowWorkspace& ws, std::size_t e,
                                      std::size_t local_byte_pos,
                                      uint8_t scratch[BLOCK4_TILE_SLOTS32_BYTES]) {
        if (ws.is_sparse[e])
            block4_sparse_unpack32(&ws.bytes[local_byte_pos], scratch);
        else
            std::memcpy(scratch, &ws.bytes[local_byte_pos], BLOCK4_TILE_SLOTS32_BYTES);
    }

    // Writes the workspace back into the shared store. `true_importance`
    // receives the raw STORED importance FLOAT directly (unlike FP4/FP8's
    // code-based callback) since float32 has no code to decode -- the
    // caller's lambda applies whatever per-row importance_scale it needs
    // directly to this value, same contract otherwise.
    template <typename TrueImportanceFn>
    std::size_t merge_row_workspace(std::size_t br, RowWorkspace& ws,
                                    TrueImportanceFn&& true_importance) {
        const std::size_t global_headroom = tile_byte_start[br + 1] - tile_byte_start[br];
            ...
            for (std::size_t e = 0; e < ws.row_nnz; ++e) {
                ...
                for (uint32_t li = 0; li < BLOCK4_TILE; ++li) {
                    for (uint32_t lj = 0; lj < BLOCK4_TILE; ++lj) {
                        ...
                        float w_val, imp_val;
                        std::memcpy(&w_val, scratch + sizeof(float) * slot, sizeof(w_val));
                        std::memcpy(&imp_val, scratch + sizeof(float) * (BLOCK4_TILE_SLOTS + slot),
                                    sizeof(imp_val));
                        ...
                        const std::size_t col =
                            static_cast<std::size_t>(ws.bc[e]) * BLOCK4_TILE + lj;
                        const double abs_imp =
                            std::abs(static_cast<double>(true_importance(row, col, imp_val)));
                        if (best_abs_imp < 0.0 || abs_imp < best_abs_imp) {
                            best_abs_imp = abs_imp;
                            best_e = e;
                            best_local_pos = pos;
                ...
                pos += block4_stored_tile_len32(ws.is_sparse[e], &ws.bytes[pos]);
            }
            if (best_abs_imp < 0.0)
                break;
            uint8_t scratch[BLOCK4_TILE_SLOTS32_BYTES];
            unpack_workspace_tile(ws, best_e, best_local_pos, scratch);
            const uint32_t best_slot = Block4Tile32::slot_index(best_li, best_lj);
            const float zero = 0.0f;
            std::memcpy(scratch + sizeof(float) * best_slot, &zero, sizeof(zero));
            std::memcpy(scratch + sizeof(float) * (BLOCK4_TILE_SLOTS + best_slot), &zero,
                        sizeof(zero));
            commit_dirty_tile_in_workspace(ws, best_e, best_local_pos, scratch);
            ++evicted;
        }
        // Same fix as Block4Store::merge_row_workspace -- see its comment
        // for the full ASan-found bug this replaced. Clamp the write-back
        // to however many WHOLE tiles fit within global_headroom instead
        // of writing back unconditionally.
        std::size_t fit_pos = 0, fit_tiles = 0;
        for (std::size_t e = 0; e < ws.row_nnz; ++e) {
            if (fit_pos >= ws.bytes.size())
                break;
            const std::size_t tlen = block4_stored_tile_len32(ws.is_sparse[e], &ws.bytes[fit_pos]);
            if (fit_pos + tlen > global_headroom)
                break;
            fit_pos += tlen;
            ++fit_tiles;
        ...
        for (std::size_t r = 0; r < block_layout.rows; ++r) {
            ...
            for (std::size_t i = start; i < end; ++i) {
                if (tile_is_sparse[i]) {
                    const uint8_t count = tile_data[byte_pos];
                    n += count;
                    byte_pos += block4_sparse_packed_len32(count);
                } else {
                    n += block4_count_live32(&tile_data[byte_pos]);
                    byte_pos += BLOCK4_TILE_SLOTS32_BYTES;
                }
            }
        }
        return n;
...
inline Block4TileHandle32::Block4TileHandle32(Block4Store32& store, uint32_t br, uint32_t bc)
    : store_(&store), br_(br), bc_(bc) {
    const std::size_t bp = store_->raw_find(br_, bc_);
    if (bp == std::numeric_limits<std::size_t>::max()) {
        valid_ = false;
    ...
    was_sparse_ = bool(store_->tile_is_sparse[elem_pos]);
    if (was_sparse_)
        block4_sparse_unpack32(&store_->tile_data[byte_pos_], scratch_);
}
...
inline Block4TileHandle32::~Block4TileHandle32() {
    if (!dirty_ || !valid_ || !was_sparse_)
        return;
    store_->commit_dirty_sparse_tile(br_, bc_, scratch_);
}

inline float Block4TileHandle32::get_weight(uint32_t li, uint32_t lj) const {
    const uint32_t slot = Block4Tile32::slot_index(li, lj);
    float v;
    std::memcpy(&v, (was_sparse_ ? scratch_ : &store_->tile_data[byte_pos_]) + sizeof(float) * slot,
                sizeof(v));
    return v;
}
inline void Block4TileHandle32::set_weight(uint32_t li, uint32_t lj, float v) {
    dirty_ = true;
    const uint32_t slot = Block4Tile32::slot_index(li, lj);
    std::memcpy((was_sparse_ ? scratch_ : &store_->tile_data[byte_pos_]) + sizeof(float) * slot, &v,
                sizeof(v));
}
inline float Block4TileHandle32::get_importance(uint32_t li, uint32_t lj) const {
    const uint32_t slot = BLOCK4_TILE_SLOTS + Block4Tile32::slot_index(li, lj);
    float v;
    std::memcpy(&v, (was_sparse_ ? scratch_ : &store_->tile_data[byte_pos_]) + sizeof(float) * slot,
                sizeof(v));
    return v;
}
inline void Block4TileHandle32::set_importance(uint32_t li, uint32_t lj, float v) {
    dirty_ = true;
    const uint32_t slot = BLOCK4_TILE_SLOTS + Block4Tile32::slot_index(li, lj);
    std::memcpy((was_sparse_ ? scratch_ : &store_->tile_data[byte_pos_]) + sizeof(float) * slot, &v,
                sizeof(v));
}
inline uint32_t Block4TileHandle32::count_live() const {
    return was_sparse_ ? block4_count_live32(scratch_)
                       : block4_count_live32(&store_->tile_data[byte_pos_]);
}
inline const uint8_t* Block4TileHandle32::raw_data() const {
    return was_sparse_ ? scratch_ : &store_->tile_data[byte_pos_];
}
inline uint8_t* Block4TileHandle32::raw_data_mutable() {
    dirty_ = true;
    return was_sparse_ ? scratch_ : &store_->tile_data[byte_pos_];
}
