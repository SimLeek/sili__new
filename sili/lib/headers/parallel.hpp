...
void omp_apply_permutation_vector_parallel(const std::vector<S>& p, std::vector<T>& vec) {
...
void omp_apply_permutation_array_parallel(const S num, const std::vector<std::size_t>& p, T* arr) {
...
void apply_permutation_to_all_array_parallel(const S num, const std::vector<std::size_t>& p,
                                             ARRAY_TYPES*... arrays) {
...
void _omp_merge_sort_vector_recursive(std::vector<TYPE>& v, unsigned long left, unsigned long right,
...
template <
    class S, class CONTAINER,
    class COMPARE = ComparatorLT<typename CONTAINER::value_type, typename CONTAINER::value_type>>
void _omp_merge_sort_array_recursive(CONTAINER& v, S left, S right,
                                     const COMPARE& cmp = COMPARE()) {
    if (left < right) {
        if (right - left >= 32) {
            S mid = (left + right) / 2;
...
void sort_indices(CSRInput<SIZE_TYPE, VALUE_TYPE>& csr, const int num_cpus = 4) {
...
#pragma omp parallel for num_threads(num_cpus) schedule(static)
    ...
    omp_merge_sort_vector(perm,
                          PermutationComparator<SIZE_TYPE*, RowColCmp>(perm.data(), row_col_cmp));

    // Apply permutation to both indices and values
    apply_permutation_to_all_vector_parallel(perm, *(csr.indices[0].get()), *(csr.values[0].get()));
}

template <typename SIZE_TYPE, typename VALUE_TYPE,
          typename Compare = ComparatorGT<VALUE_TYPE, VALUE_TYPE>>
CSRInput<SIZE_TYPE, VALUE_TYPE> top_k(const CSRInput<SIZE_TYPE, VALUE_TYPE>& tensor,
                                      const SIZE_TYPE k, const bool use_abs = true,
                                      const int num_cpus = 4, Compare cmp = Compare()) {
...
#pragma omp parallel for num_threads(num_cpus) schedule(static)
    ...
    omp_merge_sort_vector(perm,
                          PermutationComparator<SIZE_TYPE*, RowMagCmp>(perm.data(), row_mag_cmp));
