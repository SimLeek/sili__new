...
template <typename T = uint32_t> struct Synapse {
    struct ElemRef {
        uint8_t* _byte_ptr;
        bool _hi;

        operator float() const { return FP4_TABLE[_hi ? (*_byte_ptr >> 4) : (*_byte_ptr & 0xF)]; }
        ElemRef& operator=(float v) {
            const uint8_t idx = fp4_quantize(v);
            if (_hi)
                *_byte_ptr = (*_byte_ptr & 0x0Fu) | static_cast<uint8_t>(idx << 4u);
            else
                *_byte_ptr = (*_byte_ptr & 0xF0u) | idx;
            return *this;
        }
            ...
            if (new_len == old_len) {
                ...
                if (new_len > old_len && parent._growth_length < (new_len - old_len)) {
                    throw std::runtime_error(
                        "Cannot grow synapse record: no growth length remaining");
                }
    ...
    static Synapse<T> unpack(uint8_t* record, std::size_t& index, std::size_t row_start,
                             std::size_t row_end, std::size_t growth_length) {
        Synapse<T> syn;
        syn.record = record;
 ...
 */
template <typename COL_TYPE = uint32_t> struct SynapseRecord {
...
struct FP4BiPacked {
    ...
    struct ElemRef {
        ...
        ElemRef& operator=(float v) {
            const uint8_t idx = fp4_quantize(v);
            if (_hi)
                _byte = (_byte & 0x0Fu) | static_cast<uint8_t>(idx << 4u);
            else
                _byte = (_byte & 0xF0u) | idx;
            return *this;
        }
        ElemRef& operator=(const ElemRef& other) { return *this = float(other); }
        ElemRef& operator+=(float v) { return *this = float(*this) + v; }
        ...
        void resize(std::size_t n, float v = 0.0f) {
            ...
            if (v != 0.0f) {
                ...
                for (std::size_t i = old; i < n; ++i) {
                    auto& b = (**_dp)[i];
                    if (_hi)
                        b = (b & 0x0Fu) | static_cast<uint8_t>(idx << 4u);
                    else
                        b = (b & 0xF0u) | idx;
                }
            }
    ...
    void resize(std::size_t n, float weight = 0.0f, float importance = 0.0f) {
        ...
        if (weight != 0.0f || importance != 0.0f) {
            const uint8_t fill =
                static_cast<uint8_t>((fp4_quantize(weight) << 4u) | fp4_quantize(importance));
    ...
    std::vector<uint8_t> serialize() const { return _data ? *_data : std::vector<uint8_t>{}; }
