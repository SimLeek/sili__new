...
// (build once per variant via SILI_BLOCK4_FORCE_SCALAR_BACKWARD, run in
// a fresh process each time -- avoids the thermal/frequency-scaling
// confound that file's own comment documents).
//
// MEASURED (this script, 512x512 100%-dense block4, batch arg, best-of-200,
// -O3 -ffast-math -march=native):
//   batch=1:  SIMD 0.0048s vs scalar 0.0048s -- tied, no measurable win
//             or loss (the accumulate loop's own inner batch-loop only
//             runs once, so it never amortizes its own setup cost).
//   batch=32: SIMD 0.0227s vs scalar 0.0300s -- SIMD ~24% faster, a
//             real win. Confirmed via objdump that this is genuine
//             128-bit packed SIMD (vmulps/vrsqrtps/vaddps on xmm
//             registers) inside disldo_backward<..., FP8BiValues,
//             ...>'s process_tile lambda, not GCC auto-vectorizing
//             scalar code.
//
// This is WHY linear_disldo.hpp's FP8 branch decodes/encodes via scalar
// fp8_decode_bits/fp8_quantize_stochastic (not block4_vec_decode_fp8/
// block4_vec_quantize_stochastic_fp8, which measurably LOSE here --
// E4M3's 256-code space makes their subnormal/NaN-lane scalar-correction
// fallback real overhead FP4's simpler 16-code E2M1 never pays) but
// keeps the batch-loop accumulation math (RMSprop update, dx) as SIMD
// (Block4Vec) -- identical to FP4's own accumulate loop once
// weight/importance are decoded to float, and the one piece that
// measurably earns its complexity at realistic (>1) batch sizes.
//
// Build (SIMD, default):
