...
int main(int argc, char** argv) {
    ...
    w_b4.block4.switch_point = 0; // dense-loaded network: no compression, matches the planned workflow's initial state
    ...
    const double b4_fwd_ms = best_of(reps, [&]() {
        disldo_forward<SIZE_TYPE, FP4BiPacked, COL_TYPE>(x.data(), batch, n_in, w_b4, out.data(), 0.0f, 1);
    }) * 1e3;
    ...
    const double b4_bwd_ms = best_of(reps, [&]() {
        disldo_backward<SIZE_TYPE, FP4BiPacked, COL_TYPE>(
            x.data(), batch, n_in, dy.data(), w_b4, dx.data(), nia.data(), nga.data(), learning_rate, 1,
    ...
    for (auto& v : Wd) v = fp4_quantize(val_dist(rng)) | (fp4_quantize(0.5f) << 4);
    ...
    const double dense_fwd_ms = best_of(reps, [&]() {
        ...
        for (int row = 0; row < n_in; ++row) {
            ...
            for (int col = 0; col < n_out; ++col) {
                const float w = FP4_TABLE[Wrow[col] & 0xFu] * vs * output_scale[std::size_t(col)];
    ...
    const double dense_bwd_ms = best_of(reps, [&]() {
    ...
    std::printf("%-22s %10.4f %10.4f %7.2fx\n", "forward (ms)", dense_fwd_ms, b4_fwd_ms, dense_fwd_ms / b4_fwd_ms);
    std::printf("%-22s %10.4f %10.4f %7.2fx\n", "backward (ms)", dense_bwd_ms, b4_bwd_ms, dense_bwd_ms / b4_bwd_ms);
