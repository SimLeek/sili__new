...
void CHECK_CSR_WEIGHTS(
    const sparse_struct<SIZE_TYPE, CSRPointers<SIZE_TYPE>, CSRIndices<SIZE_TYPE>,
                        TriValues<VALUE_TYPE>>& weights,
    const std::tuple<std::vector<SIZE_TYPE>, std::vector<SIZE_TYPE>, std::vector<VALUE_TYPE>,
                     std::vector<VALUE_TYPE>, std::vector<VALUE_TYPE>>& expected_weights,
...
template <class SIZE_TYPE, class VALUE_TYPE>
void CHECK_COO_PROBES(const sparse_struct<SIZE_TYPE, COOPointers<SIZE_TYPE>, COOIndices<SIZE_TYPE>,
                                          UnaryValues<VALUE_TYPE>>& weights,
                      const std::tuple<std::vector<SIZE_TYPE>, std::vector<SIZE_TYPE>,
...
template <class SIZE_TYPE, class VALUE_TYPE>
void CHECK_CSR_VALUES(const sparse_struct<SIZE_TYPE, CSRPointers<SIZE_TYPE>, CSRIndices<SIZE_TYPE>,
                                          UnaryValues<VALUE_TYPE>>& weights,
                      const std::tuple<std::vector<SIZE_TYPE>, std::vector<SIZE_TYPE>,
                                       std::vector<VALUE_TYPE>>& expected_weights,
                      VALUE_TYPE diff = std::numeric_limits<VALUE_TYPE>::epsilon()) {
...
CSRSynapses<SIZE_TYPE, VALUE_TYPE>
create_test_csc(SIZE_TYPE rows, SIZE_TYPE cols, const std::vector<SIZE_TYPE>& ptrs_data,
                const std::vector<SIZE_TYPE>& indices_data,
                const std::tuple<std::vector<VALUE_TYPE>, std::vector<VALUE_TYPE>,
                                 std::vector<VALUE_TYPE>>& values_data) {
    ...
    // weights must be able to store forward multiplier strength, backprop accumulation, and optim
    // importance
    csc.values[0] = std::unique_ptr<VALUE_TYPE[]>(new VALUE_TYPE[std::get<0>(values_data).size()]);
    std::copy(std::get<0>(values_data).begin(), std::get<0>(values_data).end(),
              csc.values[0].get());
    csc.values[1] = std::unique_ptr<VALUE_TYPE[]>(new VALUE_TYPE[std::get<1>(values_data).size()]);
    std::copy(std::get<1>(values_data).begin(), std::get<1>(values_data).end(),
              csc.values[1].get());
    csc.values[2] = std::unique_ptr<VALUE_TYPE[]>(new VALUE_TYPE[std::get<2>(values_data).size()]);
    std::copy(std::get<2>(values_data).begin(), std::get<2>(values_data).end(),
              csc.values[2].get());
    return csc;
}
...
template <typename SIZE_TYPE, typename VALUE_TYPE>
CSRInput<SIZE_TYPE, VALUE_TYPE> create_test_csr_input(SIZE_TYPE rows, SIZE_TYPE cols,
...
TEST_CASE("fiber_contract_forward invalid map_ptrs", "[fiber]") {
    ...
    REQUIRE_THROWS_AS(
        fiber_contract_forward((float*)nullptr, output.data(), rows, map, import.data(), 4),
        std::invalid_argument);
    REQUIRE_THROWS_AS(
        fiber_contract_forward(expanded.data(), (float*)nullptr, rows, map, import.data(), 4),
        std::invalid_argument);
}
...
TEST_CASE("fiber_contract_backward single batch multi fiber", "[fiber]") {
    ...
    fiber_contract_backward(contracted_grad_output.data(), grad_input.data(), batches, map,
                            import.data(), 4);
...
TEST_CASE("fiber_contract_backward large fiber range", "[fiber]") {
    ...
    CHECK_VECTOR_ALMOST_EQUAL(grad_input,
                              std::vector{1.f, 1.f, 1.f, 1.f, 1.f, 2.f, 2.f, 2.f, 2.f, 2.f});
    CHECK_VECTOR_ALMOST_EQUAL(
        import, std::vector{-1.f, -1.f, -1.f, -1.f, -1.f, -2.f, -2.f, -2.f, -2.f, -2.f});
}
...
TEST_CASE("merge_duplicate_indices", "[fiber]") {
    ...
    auto weights = create_test_csc<SIZE_TYPE, VALUE_TYPE>(
        3, 3, input_ptrs, input_indices, std::tuple(input_values1, input_values2, input_values3));

    merge_duplicate_indices(weights, 4);

    std::tuple<std::vector<SIZE_TYPE>, std::vector<SIZE_TYPE>, std::vector<VALUE_TYPE>,
...
TEST_CASE("merge_duplicate_row_with_next", "[fiber]") {
    ...
    std::tuple<std::vector<SIZE_TYPE>, std::vector<SIZE_TYPE>, std::vector<VALUE_TYPE>,
        ...
        expected = {
            ...
            {29.f / 7.f, 17.f / 5.f, 5.0f, 8.0f, 7.0f, 5.0f, 8.0f,
             7.0f}, // Values[0]: averaged by importance (e.g., (1*1 + 3*3)/(1+3)=2.5)
            {7.0f, 5.0f, 9.0f, 8.0f, 7.0f, 9.0f, 8.0f, 7.0f}, // Values[1]: summed (e.g., 1+3=4)
            {29.f / 7.f, 17.f / 5.f, 5.0f, 8.0f, 7.0f, 5.0f, 8.0f, 7.0f}
            // Values[2]: averaged by value
        };
...
TEST_CASE("fiber_contract_optim_adjust_columns add true", "[fiber_optim]") {
    ...
    weights.connections = create_test_csc<SIZE_TYPE, VALUE_TYPE>(
        2, 3, ptrs, indices, std::make_tuple(values1, values2, values3));
    ;
    ...
    std::tuple<std::vector<SIZE_TYPE>, std::vector<SIZE_TYPE>, std::vector<VALUE_TYPE>,
               std::vector<VALUE_TYPE>, std::vector<VALUE_TYPE>>
        expected = {{0, 2, 3}, {0, 3, 1}, {1.f, 2.f, 3.f}, {4.f, 5.f, 6.f}, {7.f, 8.f, 9.f}};
...
TEST_CASE("fiber_contract_optim_adjust_columns add false with merge", "[fiber_optim]") {
    ...
    std::tuple<std::vector<SIZE_TYPE>, std::vector<SIZE_TYPE>, std::vector<VALUE_TYPE>,
        ...
        expected = {
            {0, 2, 3},
            {0, 1, 1},
            {1.f, 2.f, (3.f * 11.f + 4.f * 12.f) / (11.f + 12.f)},
            {5.f, 6.f, 15.f},
            {9.f, 10.f, (3.f * 11.f + 4.f * 12.f) / (3.f + 4.f)}
...
TEST_CASE("fiber_contract_optim_adjust_rows add true", "[fiber_optim]") {
    ...
    SparseLinearWeights<SIZE_TYPE, VALUE_TYPE> weights;
    weights.connections = create_test_csc<SIZE_TYPE, VALUE_TYPE>(
        3, 2, ptrs, indices, std::make_tuple(values1, values2, values3));
    std::vector<SparseLinearWeights<SIZE_TYPE, VALUE_TYPE>*> weights_array = {&weights};
    bool add = true;
    ...
    // New ptrs = ptrs[i - add_map_ptrs[i]] , for i=0: ptrs[0 -0]=0, i=1 ptrs[1-0]=2, i=2
    // ptrs[2-1]=2, i=3 ptrs[3-1]=4, i=4 ptrs[4-1]=6 ? But ptrs size 4, ptrs[4] not exist, wait.
    // Wait, new_rows =3+1=4, new_ptrs size5, i=0 to4, ptrs[i - add_map_ptrs[i]] , for i=2
    // ptrs[2-1]=ptrs[1]=2, i=3 ptrs[3-1]=ptrs[2]=4, i=4 ptrs[4-1]=ptrs[3]=6 So new_ptrs {0,2,2,4,6}
...
TEST_CASE("fiber_contract_optim_adjust_rows add false dup", "[fiber_optim]") {
    ...
    SparseLinearWeights<SIZE_TYPE, VALUE_TYPE> weights;
    weights.connections = create_test_csc<SIZE_TYPE, VALUE_TYPE>(
        3, 6, ptrs, indices, std::make_tuple(values1, values2, values3));
    std::vector<SparseLinearWeights<SIZE_TYPE, VALUE_TYPE>*> weights_array = {&weights};
    bool add = false;

    fiber_contract_optim_adjust_rows(add_map_ptrs, weights_array, add,
                                     1); // Use 1 cpu to avoid parallel issues

    CHECK(weights.connections.rows == 2);
    // new_ptrs i=0: ptrs[0+0]=0
    // i=1: ptrs[1+0]=2, but since add_map_ptrs[1]=0 == [0]=0, no merge
    // i=2: ptrs[2+1]=ptrs[3]=6, add_map_ptrs[2]=1 != [1]=0? Wait, add_map_ptrs [0,0,1,1], [2]=1 !=
    // [1]=0, yes, merge row1 (i-1=1) Merge row1 with next (row2) Slice ptrs[1]=2 to ptrs[3]=6,
    // indices {2,3,4,5}, sorted, no dup, sub=0, no shift, but sorts if not, but assume sorted. But
    // since no dup, sub=0, combined row1 now {2,3,4,5}, len4, previous row0 {0,1} new_ptrs {0,2,6}
