...
runs the toy VLM (both prefix vintages transformers has used) through
...
def run(verbose: bool = True) -> bool:
    ...
    for legacy in (False, True):
        ...
        assert len(groups) == 2, f"[{vintage}] expected 2 groups, got {len(groups)}"
        sizes = sorted(len(idxs) for _, idxs in groups)
        assert sizes == [VIS_LAYERS, TXT_LAYERS], \
            f"[{vintage}] expected sizes [{VIS_LAYERS},{TXT_LAYERS}], got {sizes}"
        for prefix, idxs in groups:
            ...
            assert len(idxs) == expected_n, \
                f"[{vintage}] prefix {prefix!r} got {len(idxs)} blocks, expected {expected_n}"
        if verbose:
            print(f"  [{vintage}] detection: 2 groups, sizes {sizes}, "
        ...
        assert len(out["folded_blocks"]) == 2, \
            f"[{vintage}] expected 2 folded blocks, got {len(out['folded_blocks'])}"
        n_folds = sorted(fb["n_folds"] for fb in out["folded_blocks"])
        assert n_folds == [VIS_LAYERS, TXT_LAYERS], \
            f"[{vintage}] expected n_folds {[VIS_LAYERS,TXT_LAYERS]}, got {n_folds}"

        remaining = [k for k in out["sparse_state_dict"] if "layers." in k]
        assert len(remaining) == 0, \
            f"[{vintage}] {len(remaining)} per-layer keys not folded: {remaining[:3]}"

        if verbose:
            print(f"  [{vintage}] fold_sparse_payload: {len(out['folded_blocks'])} "
