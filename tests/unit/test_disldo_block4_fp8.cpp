...
int main() {
    ...
    for (int r = 0; r <= 5; ++r) ptrs[r] = 0;
    idx.push_back(2); w.push_back(-1.5f); imp.push_back(0.5f);
    for (int r = 6; r <= n_in; ++r) ptrs[r] = 1;
    ...
    CHECK(weights.block4.n_tiles() == 1, "expected exactly 1 block4 tile, got %zu", weights.block4.n_tiles());
    ...
    for (int c = 0; c < n_out; ++c)
        CHECK(std::abs(y[c] - y_ref[c]) < 1e-4f, "forward output[%d]: got %.4f expected %.4f", c, y[c], y_ref[c]);
    ...
    // Regression: with block4 empty, output must match scattered-only.
    {
        ...
        CHECK(std::abs(y2[2] - (-1.5f)) < 1e-4f, "scattered-only regression: output[2] got %.4f expected -1.5", y2[2]);
        CHECK(std::abs(y2[1]) < 1e-6f, "scattered-only regression: output[1] should be 0 (no block4), got %.4f", y2[1]);
    }
    ...
    disldo_backward<int, FP8BiValues, uint32_t>(
        x.data(), 1, n_in, dy.data(), weights, dx.data(), nia.data(), nga.data(),
        0.05f, 1, false, true);

    CHECK(weights.block4.n_tiles() == 1, "tile should still exist after backward, got %zu tiles", weights.block4.n_tiles());
...

    {
        ...
        CHECK(std::isfinite(w_after) && std::isfinite(imp_after), "weight/importance not finite after backward");
        CHECK(w_after != w_before || imp_after != imp_before,
              "backward should have changed (row=0,col=1)'s weight or importance -- w %.4f->%.4f imp %.4f->%.4f",
