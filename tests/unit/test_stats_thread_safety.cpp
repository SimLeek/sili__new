...
static SparseLinearWeightsDelta<int, FP4BiPacked, uint32_t>
build_test_layer(const std::vector<int>& ptrs, const std::vector<int>& idx,
                 const std::vector<float>& w0, const std::vector<float>& imp0, std::size_t rows,
                 std::size_t cols) {
...
TEST_CASE("delta_csr_synap_row_step's row cursor advances even when the merge itself throws",
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        try {
            delta_csr_synap_row_step<SIZE_TYPE, FP4BiPacked, COL_TYPE>(weights, cursor, -1e9f,
                                                                       SIZE_TYPE(10));
...
TEST_CASE(
    "disldo_forward/backward: num_cpus=1 and num_cpus=8 give matching stats pre_existing_failure",
    ...
    std::vector<S> ptrs(201, 0), idx;
    std::vector<float> w0, imp0;
    for (int r = 0; r < 200; ++r) {
        std::set<int> cols;
        while ((int)cols.size() < 10)
            cols.insert(col_dist(rng));
        for (int c : cols) {
            idx.push_back(c);
            w0.push_back(val_dist(rng));
            imp0.push_back(0.0f);
        }
        ptrs[r + 1] = (S)idx.size();
    }
    ...
    auto run_trial = [&](int num_cpus) {
        ...
        for (int step = 0; step < 10; ++step) {
            ...
            for (auto& v : input)
                v = d(trial_rng);
            disldo_forward<S, FP4BiPacked, COL_TYPE>(input.data(), S(1), S(200), weights,
                                                     output.data(), num_cpus);
            std::vector<float> dy(200), dx(200, 0.0f), in_acc(200, 0.0f), gr_acc(200, 0.0f);
            for (auto& v : dy)
                v = d(trial_rng);
            disldo_backward<S, FP4BiPacked, COL_TYPE>(input.data(), S(1), S(200), dy.data(),
                                                      weights, dx.data(), in_acc.data(),
                                                      gr_acc.data(), 0.3f, num_cpus);
        }
        return weights;
    };
