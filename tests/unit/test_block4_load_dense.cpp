...
int main() {
    ...
    weights.connections.layout.cols =
        n_out; // not from .connections -- these two lines just mirror
    ...
    // ── nnz / tile population ───────────────────────────────────────────
    CHECK(weights.block4.live_synapses() == n_in * n_out, "live_synapses should be %zu, got %zu",
          n_in * n_out, weights.block4.live_synapses());
    ...
    for (uint32_t br = 0; br < block_rows; ++br) {
        for (uint32_t bc = 0; bc < block_cols; ++bc) {
            ...
            CHECK(tile && tile.count_live() == 16,
                  "tile (%u,%u) should be fully dense (16 live), got %u", br, bc,
    ...
    for (std::size_t row = 0; row < n_in && codes_match; ++row) {
        for (std::size_t col = 0; col < n_out && codes_match; ++col) {
            ...
            if (stored != expected) {
                codes_match = false;
                CHECK(false, "row=%zu col=%zu: stored code %u != expected %u", row, col, stored,
                      expected);
            }
        }
    }
    ...
    disldo_forward<SIZE_TYPE, FP4BiPacked, COL_TYPE>(x.data(), 1, SIZE_TYPE(n_in), weights,
                                                     y.data(), 1);
    for (std::size_t c = 0; c < n_out; ++c) {
        const float expected = FP4_TABLE[weight_codes[c]];
        CHECK(std::abs(y[c] - expected) < 1e-4f,
              "forward output[%zu] should be %.4f (decoded code %u), got %.4f", c, expected,
              weight_codes[c], y[c]);
    }
