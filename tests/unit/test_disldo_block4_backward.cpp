...
int main() {
    ...
    disldo_backward<int, FP4BiPacked, uint32_t>(x.data(), 1, n_in, dy.data(), weights, dx.data(),
                                                neuron_in.data(), neuron_grad.data(),
                                                /*learning_rate=*/0.0f, /*num_cpus=*/1);

    for (int r = 0; r < n_in; ++r)
        CHECK(std::abs(dx[r] - dx_ref[r]) < 1e-4f, "dx[%d]: got %.4f expected %.4f", r, dx[r],
              dx_ref[r]);
    ...
    disldo_backward<int, FP4BiPacked, uint32_t>(
        x.data(), 1, n_in, dy.data(), weights, dx2.data(), ni2.data(), ng2.data(),
    ...
    CHECK(w01_after < 1.0f - 1e-6f,
          "block4 (0,1) weight should decrease under lr=0.5, g>0: got %.3f", w01_after);

    // (row=2,col=3): input=2.0, dy=-0.2 -> g=-0.4 -> weight should move UP.
    const float w23_after = FP4_TABLE[t_after1.at(2, 3) & 0xFu];
    CHECK(w23_after > 0.5f - 1e-6f,
          "block4 (2,3) weight should increase under lr=0.5, g<0: got %.3f", w23_after);
