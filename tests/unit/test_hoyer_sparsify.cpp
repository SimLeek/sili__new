...
TEST_CASE("hoyer_sparsify_row: all-zero vector doesn't NaN, defines k=0", "[hoyer][edge_case]") {
...
TEST_CASE("hoyer_sparsify_per_batch: per-row k_estimate genuinely differs, not shared across batch",
    ...
    batch[0] = 1.0f;
    batch[1] = 1.0f; // row 0: 2 nonzero
    for (int i = 0; i < 8; ++i)
        batch[10 + i] = 1.0f; // row 1: 8 nonzero
...
TEST_CASE("hoyer_score: distinguishes a mostly-sparse batch from a mostly-dense one",
    ...
    for (int r = 0; r < 5; ++r)
        for (int i = 0; i < 3; ++i)
            mostly_sparse[r * 100 + i] = 1.0f; // 3/100 per row, every row
