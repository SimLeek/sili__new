...
// CSR (sisldo_forward/disldo_backward_sparse_grad).
//
// Mirrors test_sisldo_disldo_parity.cpp exactly, VALUES_TYPE=FP8BiValues
// instead of FP4BiPacked (fp8_quantize/fp8_decode_bits, no nibble packing --
// FP8 codes are a full byte per weight, no importance-in-high-nibble
// convention). AQRS sweep kept (unlike the fp32 parity test, which drops it
// since fp32 never uses AQRS in this project) -- FP8 genuinely uses the
// additive branch in production (the "mumbling" fix, tasks #274-295).
//
// The [block4] configs are the ones that specifically catch
// docs/research/sisldo_ops.rst:
// sisldo_forward.block4_fp4_only_silent_zero_for_dense_fp32_fp8 -- until
// that fix, sisldo_forward/disldo_backward_sparse_grad's block4 branch was
// gated `if constexpr FP4BiPacked`-only, so a dense=True FP8 layer's
// block4-resident synapses were never read/written at all here.
...
static Weights make_weights(const std::vector<uint8_t>& weight_codes, std::size_t n_in,
                            std::size_t n_out, std::size_t scale_rank, std::size_t additive_rank,
    ...
    for (std::size_t r = 0; r < n_in; ++r) {
        ...
        for (std::size_t c = 0; c < n_out; ++c) {
            ...
            wv[r * n_out + c] = fp8_decode_bits(weight_codes[r * n_out + c]);
        }
    }
...
// Block4-resident variant -- same weight codes, loaded via block4_load_dense
// instead of delta_csr_from_absolute.
static Weights make_weights_block4(const std::vector<uint8_t>& weight_codes, std::size_t n_in,
                                   std::size_t n_out, std::size_t scale_rank,
                                   std::size_t additive_rank, bool seed_additive,
    ...
    block4_load_dense<SIZE_TYPE, FP8BiValues, COL_TYPE>(w, weight_codes.data(),
...
static void run_config(const char* name, std::size_t scale_rank, std::size_t additive_rank,
    ...
    std::printf("--- config: %s (scale_rank=%zu additive_rank=%zu gamma_trainable=%d) ---\n", name,
                scale_rank, additive_rank, int(gamma_trainable));
    ...
    Weights weights_dense =
        make_weights(weight_codes, n_in, n_out, scale_rank, additive_rank, true, gamma_trainable);
    Weights weights_sparse =
        make_weights(weight_codes, n_in, n_out, scale_rank, additive_rank, true, gamma_trainable);

    std::vector<float> y_dense(n_out, 0.0f), y_sparse(n_out, 0.0f);
    disldo_forward<SIZE_TYPE, FP8BiValues, COL_TYPE>(input.data(), 1, SIZE_TYPE(n_in),
                                                     weights_dense, y_dense.data(), 1);
    sisldo_forward<SIZE_TYPE, FP8BiValues, COL_TYPE>(in_csr, weights_sparse, y_sparse.data(), 1);
    for (std::size_t c = 0; c < n_out; ++c)
    ...
    disldo_backward<SIZE_TYPE, FP8BiValues, COL_TYPE, RMSpropScalePolicy<float>, false, false>(
        ...
        ng_d.data(), lr, 1);
    disldo_backward_sparse_grad<SIZE_TYPE, FP8BiValues, COL_TYPE, RMSpropScalePolicy<float>, false>(
    ...
    for (std::size_t r = 0; r < n_in; ++r) {
        ...
        for (std::size_t e = 0; e < row_nnz; ++e) {
            ...
            const float w_dense =
                ValueAccessor<FP8BiValues>::get_w(weights_dense.connections.values, vb);
            const float w_sparse =
                ValueAccessor<FP8BiValues>::get_w(weights_sparse.connections.values, vb);
    ...
    for (std::size_t r = 0; r < n_in; ++r)
        for (std::size_t k = 0; k < scale_rank; ++k)
            CHECK(std::abs(weights_dense.get_value_scale_k(r, k) -
                           weights_sparse.get_value_scale_k(r, k)) < 1e-3f,
                  "value_scale[%zu][k=%zu] diverges: dense=%.6f sparse=%.6f", r, k,
                  weights_dense.get_value_scale_k(r, k), weights_sparse.get_value_scale_k(r, k));
    for (std::size_t c = 0; c < n_out; ++c)
        for (std::size_t k = 0; k < scale_rank; ++k)
            CHECK(std::abs(weights_dense.get_output_scale_k(c, k) -
                           weights_sparse.get_output_scale_k(c, k)) < 1e-3f,
                  "output_scale[%zu][k=%zu] diverges: dense=%.6f sparse=%.6f", c, k,
                  weights_dense.get_output_scale_k(c, k), weights_sparse.get_output_scale_k(c, k));

    if (additive_rank > 0) {
        for (std::size_t r = 0; r < n_in; ++r)
            for (std::size_t k = 0; k < additive_rank; ++k)
                CHECK(std::abs(weights_dense.get_additive_u_k(r, k) -
                               weights_sparse.get_additive_u_k(r, k)) < 1e-3f,
                      "additive_u[%zu][k=%zu] diverges: dense=%.6f sparse=%.6f", r, k,
                      weights_dense.get_additive_u_k(r, k), weights_sparse.get_additive_u_k(r, k));
        for (std::size_t c = 0; c < n_out; ++c)
            for (std::size_t k = 0; k < additive_rank; ++k)
                CHECK(std::abs(weights_dense.get_additive_v_k(c, k) -
                               weights_sparse.get_additive_v_k(c, k)) < 1e-3f,
                      "additive_v[%zu][k=%zu] diverges: dense=%.6f sparse=%.6f", c, k,
                      weights_dense.get_additive_v_k(c, k), weights_sparse.get_additive_v_k(c, k));
        for (std::size_t k = 0; k < additive_rank; ++k)
            CHECK(std::abs(weights_dense.get_additive_gamma_k(k) -
                           weights_sparse.get_additive_gamma_k(k)) < 1e-3f,
                  "additive_gamma[k=%zu] diverges: dense=%.6f sparse=%.6f", k,
                  weights_dense.get_additive_gamma_k(k), weights_sparse.get_additive_gamma_k(k));
    }
    if (scale_rank > 1 && gamma_trainable) {
        for (std::size_t k = 0; k < scale_rank; ++k)
            CHECK(std::abs(weights_dense.get_scale_gamma_k(k) -
                           weights_sparse.get_scale_gamma_k(k)) < 1e-3f,
                  "scale_gamma[k=%zu] diverges: dense=%.6f sparse=%.6f", k,
                  weights_dense.get_scale_gamma_k(k), weights_sparse.get_scale_gamma_k(k));
    }
}

// Block4-resident sweep -- the config that catches
// sisldo_forward.block4_fp4_only_silent_zero_for_dense_fp32_fp8 for FP8.
static void run_config_block4(const char* name, std::size_t scale_rank, std::size_t additive_rank,
                              bool gamma_trainable) {
    std::printf(
        "--- config: %s [block4] (scale_rank=%zu additive_rank=%zu gamma_trainable=%d) ---\n", name,
        scale_rank, additive_rank, int(gamma_trainable));
    ...
    Weights weights_dense = make_weights_block4(weight_codes, n_in, n_out, scale_rank,
                                                additive_rank, true, gamma_trainable);
    Weights weights_sparse = make_weights_block4(weight_codes, n_in, n_out, scale_rank,
                                                 additive_rank, true, gamma_trainable);
    ...
    for (std::size_t r = 0; r < n_in; ++r)
        for (std::size_t c = 0; c < n_out; ++c) {
            ...
            const float w_dense =
                fp8_decode_bits(weights_dense.block4.find(br, bc).at_weight(li, lj));
            const float w_sparse =
                fp8_decode_bits(weights_sparse.block4.find(br, bc).at_weight(li, lj));
    ...
    for (std::size_t r = 0; r < n_in; ++r)
        for (std::size_t k = 0; k < scale_rank; ++k)
            CHECK(std::abs(weights_dense.get_value_scale_k(r, k) -
                           weights_sparse.get_value_scale_k(r, k)) < 1e-3f,
                  "[block4] value_scale[%zu][k=%zu] diverges: dense=%.6f sparse=%.6f", r, k,
                  weights_dense.get_value_scale_k(r, k), weights_sparse.get_value_scale_k(r, k));
    for (std::size_t c = 0; c < n_out; ++c)
        for (std::size_t k = 0; k < scale_rank; ++k)
            CHECK(std::abs(weights_dense.get_output_scale_k(c, k) -
                           weights_sparse.get_output_scale_k(c, k)) < 1e-3f,
                  "[block4] output_scale[%zu][k=%zu] diverges: dense=%.6f sparse=%.6f", c, k,
                  weights_dense.get_output_scale_k(c, k), weights_sparse.get_output_scale_k(c, k));
    ...
    if (additive_rank > 0) {
        for (std::size_t r = 0; r < n_in; ++r)
            for (std::size_t k = 0; k < additive_rank; ++k)
                CHECK(std::abs(weights_dense.get_additive_u_k(r, k) -
                               weights_sparse.get_additive_u_k(r, k)) < 1e-3f,
                      "[block4] additive_u[%zu][k=%zu] diverges: dense=%.6f sparse=%.6f", r, k,
                      weights_dense.get_additive_u_k(r, k), weights_sparse.get_additive_u_k(r, k));
        for (std::size_t c = 0; c < n_out; ++c)
            for (std::size_t k = 0; k < additive_rank; ++k)
                CHECK(std::abs(weights_dense.get_additive_v_k(c, k) -
                               weights_sparse.get_additive_v_k(c, k)) < 1e-3f,
                      "[block4] additive_v[%zu][k=%zu] diverges: dense=%.6f sparse=%.6f", c, k,
                      weights_dense.get_additive_v_k(c, k), weights_sparse.get_additive_v_k(c, k));
        for (std::size_t k = 0; k < additive_rank; ++k)
            CHECK(std::abs(weights_dense.get_additive_gamma_k(k) -
                           weights_sparse.get_additive_gamma_k(k)) < 1e-3f,
                  "[block4] additive_gamma[k=%zu] diverges: dense=%.6f sparse=%.6f", k,
                  weights_dense.get_additive_gamma_k(k), weights_sparse.get_additive_gamma_k(k));
    }
    if (scale_rank > 1 && gamma_trainable) {
        for (std::size_t k = 0; k < scale_rank; ++k)
            CHECK(std::abs(weights_dense.get_scale_gamma_k(k) -
                           weights_sparse.get_scale_gamma_k(k)) < 1e-3f,
                  "[block4] scale_gamma[k=%zu] diverges: dense=%.6f sparse=%.6f", k,
                  weights_dense.get_scale_gamma_k(k), weights_sparse.get_scale_gamma_k(k));
    }
}
