...
static bool is_nearest_or_tied(float v, uint8_t code) {
    ...
    for (int i = 0; i < 256; ++i) {
        if (i == 0x7F || i == 0xFF)
            continue; // both NaN slots (sign doesn't matter for NaN)
...
int main() {
    for (int code = 0; code < 256; ++code)
        g_decoded[code] = fp8_decode_bits(uint8_t(code));
    ...
    for (int code = 0; code < 256; ++code) {
        ...
        if (is_nan_slot) {
            ...
            CHECK(std::isfinite(g_decoded[code]), "decode(%d) = %f not finite", code,
                  double(g_decoded[code]));
        }
    }
    for (int m = 0; m < 0x7F; ++m) {
        if ((m & 0x7F) == 0x7F)
            continue;
        if (m > 0)
            CHECK(g_decoded[m] >= g_decoded[m - 1] - 1e-12f,
                  "decode magnitude not monotonic at code %d: %f then %f", m,
                  double(g_decoded[m - 1]), double(g_decoded[m]));
    }
    CHECK(g_decoded[0] == 0.0f, "decode(0) should be exactly 0.0, got %f", double(g_decoded[0]));
    CHECK(g_decoded[0x7E] == 448.0f, "decode(0x7E) (max finite) should be 448.0, got %f",
          double(g_decoded[0x7E]));
    ...
    for (int i = -9000; i <= 9000; ++i) {
        ...
        CHECK((code & 0x7F) != 0x7F, "encode(%f) produced a NaN slot for a finite input",
              double(v));
        CHECK(is_nearest_or_tied(v, code), "encode(%f) = %d (%f) is not nearest", double(v),
              int(code), double(g_decoded[code]));
        ++tested;
    }
    ...
    const float boundaries[] = {0.0f,    -0.0f,     448.0f,     -448.0f,   500.0f,
                                -500.0f, 100000.0f, -100000.0f, 0.015625f, -0.015625f,
                                0.001f,  -0.001f,   1.0f,       -1.0f};
    ...
    CHECK(fp8_encode_bits(1e9f) == 0x7E,
          "encode(large finite) should saturate to max (0x7E), got %d", int(fp8_encode_bits(1e9f)));
    CHECK(fp8_encode_bits(-1e9f) == 0xFE,
          "encode(-large finite) should saturate to -max (0xFE), got %d",
          int(fp8_encode_bits(-1e9f)));
    ...
    for (int code = 0; code < 256; ++code) {
        ...
        CHECK(re == uint8_t(code) || g_decoded[re] == v,
              "round-trip broke at code %d (%f): re-encoded to %d (%f)", code, double(v), int(re),
              double(g_decoded[re]));
    }
    ...
    // v), matches fp4_quantize_stochastic's own test convention.
    {
        ...
        const int n = 200000;
        for (int i = 0; i < n; ++i)
            sum += fp8_decode_bits(fp8_quantize_stochastic(v));
