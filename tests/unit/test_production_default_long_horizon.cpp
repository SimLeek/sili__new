...
int main() {
    ...
    for (std::size_t step = 0; step < N_STEPS; ++step) {
        for (std::size_t i = 0; i < N; ++i) {
            ...
            disldo_backward<SIZE_TYPE, FP4BiPacked, COL_TYPE, RMSpropScalePolicy<float>, false,
                            true, BoundedRMSpropSynapsePolicy>(
                &basis[i * N], 1, SIZE_TYPE(N), dy.data(), weights, dx.data(), ni.data(), ng.data(),
                LR, 1, false, true, 0.999f, 1e-8f, BETA1, MIN_DECAY_FRAC, MAX_ABS_DELTA, MAX_CI);
        }
        ...
        if (!std::isfinite(sse) || !std::isfinite(max_abs_y))
            saw_non_finite = true;
        if (step >= 200)
            max_sse_after_200 = std::max(max_sse_after_200, sse);
    ...
    CHECK(!saw_non_finite,
          "production default produced a non-finite SSE or output somewhere in %zu steps", N_STEPS);
    ...
    CHECK(max_sse_after_200 < 50.0f,
          "production default's SSE exceeded the long-horizon stability ceiling after step 200: "
          "max=%.4f (limit 50.0)",
          max_sse_after_200);
    ...
    if (g_fail == 0) {
        std::printf("test_production_default_long_horizon: production default stayed stable for "
                    "the full %zu-step run.\n",
                    N_STEPS);
        return 0;
    }
