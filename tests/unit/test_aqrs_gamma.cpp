...
static void test_gamma_default_backward_compat(bool use_block4) {
    ...
    for (std::size_t r = 0; r < N; ++r) w.set_value_scale_raw_k(r, 0, 2.5f * float(r + 1));
    for (std::size_t c = 0; c < N; ++c) w.set_output_scale_raw_k(c, 0, 1.5f);

    CHECK(std::fabs(w.get_scale_gamma_k(0) - 1.0f) < 1e-6f,
          "[block4=%d] gamma_s_k(0) must default to 1.0 (identity multiplier for the legacy always-on channel), got %.6f",
    ...
    for (std::size_t r = 0; r < N; ++r) {
        ...
        CHECK(std::fabs(y[r * N + r] - expected) < 1e-2f,
              "[block4=%d] row %zu diagonal output should match pre-gamma S formula exactly (got %.6f expected %.6f)",
...
static void test_gamma_zero_is_true_off_switch(bool use_block4) {
    ...
    w.scale_rank = 1;
    w.set_additive_rank(0);
    for (std::size_t r = 0; r < N; ++r) w.set_value_scale_raw_k(r, 0, 3.0f);
    ...
    CHECK(std::fabs(w.get_scale_gamma_k(1) - 0.0f) < 1e-6f,
          "[block4=%d] gamma_s_k(1) (newly grown channel, gamma already in active use) must default to 0.0, got %.6f",
    ...
    for (std::size_t r = 0; r < N; ++r) w.set_value_scale_raw_k(r, 1, 10.0f * float(r + 1));
    for (std::size_t c = 0; c < N; ++c) w.set_output_scale_raw_k(c, 1, 7.0f);

    std::vector<float> y_after = forward_only(w);
    CHECK(all_close(y_before, y_after),
          "[block4=%d] gamma_s_k(1)=0 must zero the whole channel 1 contribution regardless of how large value_scale/output_scale k=1 are",
          int(use_block4));
}
...
static void test_gamma_backward_moves_toward_loss_reduction(bool use_block4) {
    ...
    CHECK(final_gamma > 3.0f,
          "[block4=%d] gamma should have grown substantially toward the target scale via its own gradient (got %.4f)",
          int(use_block4), final_gamma);
}
...
static void test_gamma_l1_fixed_point_at_zero(bool use_block4) {
    ...
    for (std::size_t i = 0; i < w_q.size(); ++i) w_star[i] = 3.0f * w_q[i];  // 3x w_q
    ...
    auto freeze_channel0_and_direction1 = [&]() {
        ...
        for (std::size_t c = 0; c < N; ++c) w.set_output_scale_raw_k(c, 0, 1.0f);
        w.set_scale_gamma_raw_k(0, 1.0f);
        for (std::size_t r = 0; r < N; ++r) w.set_value_scale_raw_k(r, 1, 1.0f);
        for (std::size_t c = 0; c < N; ++c) w.set_output_scale_raw_k(c, 1, 1.0f);
    };
    ...
    std::printf("[gamma-L1 block4=%d] final gamma_s_k(1): %.6f (seeded 0.5, L1 should pull to exactly 0)\n", int(use_block4), gamma1_final);
    CHECK(std::fabs(gamma1_final) < 1e-4f,
          "[block4=%d] L1 penalty should pull an unneeded channel's gamma to exactly 0, a real fixed point (got %.6f)",
          int(use_block4), gamma1_final);

    // Run MORE steps -- a true fixed point must stay settled, not drift.
    run_steps(1000);
    CHECK(std::fabs(w.get_scale_gamma_k(1)) < 1e-4f,
          "[block4=%d] gamma_s_k(1) must STAY at 0 (real fixed point), not drift back up (got %.6f after extra steps)",
          int(use_block4), w.get_scale_gamma_k(1));
}
