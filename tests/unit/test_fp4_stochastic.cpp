...
int main() {
    ...
    const float test_vals[] = {0.05f, 0.1f,  0.2f, 0.3f, 0.4f, 0.6f,  0.7f,  0.8f,  0.9f,
                               1.1f,  1.25f, 1.4f, 1.6f, 1.9f, 2.2f,  2.6f,  2.9f,  3.2f,
                               3.7f,  4.5f,  5.0f, 5.5f, 5.9f, -0.3f, -1.7f, -3.3f, -5.8f};
    for (float v : test_vals) {
        double sum = 0.0;
        for (int i = 0; i < N; ++i)
            sum += double(fp4_decode_bits(fp4_quantize_stochastic(v)));
        const double mean = sum / N;
        // Worst-case single-gap width is 2.0 (the 4<->6 gap); N=200000
        // easily clears 0.02 unless something is systematically biased.
        CHECK(std::abs(mean - v) <= 0.02, "E[stochastic(%f)] = %f, off by %f", double(v), mean,
              mean - v);
    }
    ...
    for (int i = -1200; i <= 1200; ++i) {
        ...
        CHECK(fp4_quantize_stochastic(v) != 8, "produced the NaN slot (8) for finite v=%f",
              double(v));
    }
