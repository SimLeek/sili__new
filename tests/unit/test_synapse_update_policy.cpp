...
TEST_CASE("PlainRMSpropSynapsePolicy::update_cw applies S correctly", "[synapse_policy][plain]") {
    using P = PlainRMSpropSynapsePolicy<float>;
...
TEST_CASE("PlainRMSpropSynapsePolicy::update_cw's scale_invariant flag makes the "
    ...
    const float g = 0.5f, ci = 0.1f, eff_lr = 0.01f, eps = 1e-8f;
    const float dcw_S1 =
        P::update_cw(g, ci, 1.0f, eff_lr, eps, true, 1e30f, /*scale_invariant=*/true);
    const float dcw_S4 =
        P::update_cw(g, ci, 4.0f, eff_lr, eps, true, 1e30f, /*scale_invariant=*/true);
    ...
    const float dcw_S1_old =
        P::update_cw(g, ci, 1.0f, eff_lr, eps, true, 1e30f, /*scale_invariant=*/false);
    const float dcw_S4_old =
        P::update_cw(g, ci, 4.0f, eff_lr, eps, true, 1e30f, /*scale_invariant=*/false);
...
TEST_CASE(
    "BoundedRMSpropSynapsePolicy::update_ci's max_ci and min_decay_frac floor compose correctly",
    "[synapse_policy][bounded][max_ci]") {
...
TEST_CASE("BoundedRMSpropSynapsePolicy::update_cw matches plain RMSprop delta when under the clip",
    ...
    const float bounded =
        P::update_cw(g, ci, S, eff_lr, eps, true, 1e30f); // max_abs_delta huge, no clip
...
TEST_CASE("BoundedRMSpropSynapsePolicy::update_cw's clip actually engages on a real large step",
    ...
    REQUIRE(std::abs(unclipped_raw_would_be) >
            max_abs_delta); // sanity: this really would overshoot without the clip
...
TEST_CASE("BoundedRMSpropSynapsePolicy::update_cw's clip preserves sign",
    ...
    CHECK(delta_pos_g ==
          Catch::Approx(-eff_lr * max_abs_delta)); // positive g -> negative (descent) delta
...
TEST_CASE("BoundedRMSpropSynapsePolicy::update_cw's scale_invariant flag makes the "
    ...
    const float dcw_S1 =
        P::update_cw(g, ci, 1.0f, eff_lr, eps, true, max_abs_delta, /*scale_invariant=*/true);
    const float dcw_S4 =
        P::update_cw(g, ci, 4.0f, eff_lr, eps, true, max_abs_delta, /*scale_invariant=*/true);
    CHECK(1.0f * dcw_S1 == Catch::Approx(4.0f * dcw_S4));

    // Old (default/false) formula: quadratic-in-S collapse still present.
    const float dcw_S1_old =
        P::update_cw(g, ci, 1.0f, eff_lr, eps, true, max_abs_delta, /*scale_invariant=*/false);
    const float dcw_S4_old =
        P::update_cw(g, ci, 4.0f, eff_lr, eps, true, max_abs_delta, /*scale_invariant=*/false);
    ...
    const float clipped_S1 =
        P::update_cw(g, ci, 1.0f, eff_lr, eps, true, tight_clip, /*scale_invariant=*/true);
    const float clipped_S4 =
        P::update_cw(g, ci, 4.0f, eff_lr, eps, true, tight_clip, /*scale_invariant=*/true);
...
TEST_CASE("update_ci/update_cw match linear_disldo.hpp's existing inline formula exactly",
    ...
    const float ci_policy =
        P::update_ci(0.2f, g, contrib, beta2, /*min_decay_frac=*/0.0f, /*max_ci=*/1e30f);
...
TEST_CASE("RMSpropScalePolicy::update's log_space flag makes the RELATIVE "
    ...
    uint32_t step_a = 0;
    P::update(scale_a, state_a, g_agg, eff_lr, beta2, eps, contrib_agg, &step_a,
              /*log_space=*/true);
    const float rel_change_a = scale_a / 1.0f;

    float scale_b = 0.01f, state_b = 0.0f;
    uint32_t step_b = 0;
    P::update(scale_b, state_b, g_agg, eff_lr, beta2, eps, contrib_agg, &step_b,
              /*log_space=*/true);
    ...
    uint32_t step_c = 0;
    P::update(scale_c, state_c, g_agg, eff_lr, beta2, eps, contrib_agg, &step_c,
              /*log_space=*/false);
    const float rel_change_c = scale_c / 1.0f;

    float scale_d = 0.01f, state_d = 0.0f;
    uint32_t step_d = 0;
    P::update(scale_d, state_d, g_agg, eff_lr, beta2, eps, contrib_agg, &step_d,
              /*log_space=*/false);
