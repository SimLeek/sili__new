...
static void test_additive_gamma_default_backward_compat(bool use_block4) {
    ...
    for (std::size_t r = 0; r < N; ++r)
        w.set_additive_u_raw_k(r, 0, 1.0f);
    for (std::size_t c = 0; c < N; ++c)
        w.set_additive_v_raw_k(c, 0, float(c + 1));
    ...
    for (std::size_t r = 0; r < N; ++r) {
        for (std::size_t c = 0; c < N; ++c) {
            ...
            CHECK(std::fabs(y[r * N + c] - expected) < 1e-2f,
                  "[block4=%d] row %zu col %zu should match pre-gamma additive formula exactly "
                  "(got %.6f expected %.6f)",
...
static void test_additive_gamma_zero_is_true_off_switch(bool use_block4) {
    ...
    w.set_additive_rank(1);
    for (std::size_t r = 0; r < N; ++r)
        w.set_additive_u_raw_k(r, 0, 3.0f);
    for (std::size_t c = 0; c < N; ++c)
        w.set_additive_v_raw_k(c, 0, 2.0f);
    ...
    CHECK(std::fabs(w.get_additive_gamma_k(1) - 0.0f) < 1e-6f,
          "[block4=%d] additive_gamma_k(1) (newly grown channel, gamma already active) must "
          "default to 0.0, got %.6f",
          int(use_block4), w.get_additive_gamma_k(1));
    for (std::size_t r = 0; r < N; ++r)
        w.set_additive_u_raw_k(r, 1, 10.0f * float(r + 1));
    for (std::size_t c = 0; c < N; ++c)
        w.set_additive_v_raw_k(c, 1, 7.0f);

    std::vector<float> y_after = forward_only(w);
    CHECK(all_close(y_before, y_after),
          "[block4=%d] additive_gamma_k(1)=0 must zero the whole channel 1 contribution regardless "
          "of u/v magnitude",
          int(use_block4));
}
...
static void test_additive_gamma_backward_moves_toward_loss_reduction(bool use_block4) {
    ...
    for (std::size_t r = 0; r < N; ++r)
        w.set_additive_u_raw_k(r, 0, 1.0f);
    for (std::size_t c = 0; c < N; ++c)
        w.set_additive_v_raw_k(c, 0, 1.0f);
    ...
    for (int step = 0; step < 2000; ++step) {
        for (std::size_t i = 0; i < N; ++i) {
            ...
            for (std::size_t c = 0; c < N; ++c)
                w.set_additive_v_raw_k(c, 0, 1.0f);
            for (std::size_t r = 0; r < N; ++r)
                w.set_value_scale_raw_k(r, 0, 0.0f);
        }
    }
    const float final_gamma = w.get_additive_gamma_k(0);
    std::printf(
        "[additive-gamma-backward block4=%d] final gamma_o_k(0): %.4f (seeded 0.1, target 1.25)\n",
...
static void test_additive_gamma_l1_fixed_point_at_zero_including_k0(bool use_block4) {
    ...
    auto freeze_channel0_and_direction1 = [&]() {
        for (std::size_t r = 0; r < N; ++r)
            w.set_additive_u_raw_k(r, 0, 1.0f);
        for (std::size_t c = 0; c < N; ++c)
            w.set_additive_v_raw_k(c, 0, 5.0f);
        w.set_additive_gamma_raw_k(0, 1.0f);
        for (std::size_t r = 0; r < N; ++r)
            w.set_additive_u_raw_k(r, 1, 1.0f);
        for (std::size_t c = 0; c < N; ++c)
            w.set_additive_v_raw_k(c, 1, 1.0f);
        for (std::size_t r = 0; r < N; ++r)
            w.set_value_scale_raw_k(r, 0, 0.0f);
    };
    ...
    std::printf("[additive-gamma-L1 block4=%d] final gamma_o_k(1): %.6f (seeded 0.5, L1 should "
                "pull to exactly 0)\n",
                int(use_block4), gamma1_final);
    CHECK(std::fabs(gamma1_final) < 1e-4f,
          "[block4=%d] L1 penalty should pull an unneeded channel's gamma to exactly 0 (additive "
          "branch has no k==0 exemption -- channel 1 here, not the always-on baseline, is the one "
          "under test), got %.6f",
          int(use_block4), gamma1_final);

    run_steps(1000);
    CHECK(std::fabs(w.get_additive_gamma_k(1)) < 1e-4f,
          "[block4=%d] additive_gamma_k(1) must STAY at 0 (real fixed point), not drift back up "
          "(got %.6f after extra steps)",
          int(use_block4), w.get_additive_gamma_k(1));
}
...
static void test_additive_gamma_block4_scattered_parity() {
    ...
    CHECK(all_close(y_scattered, y_block4, 1e-2f),
          "block4 and scattered must produce identical additive-branch-inclusive forward output "
    ...
    disldo_backward<SIZE_TYPE, FP4BiPacked, COL_TYPE, RMSpropScalePolicy<float>, false, false>(
        basis.data(), 1, SIZE_TYPE(N), dy.data(), w_scattered, dx1.data(), ni1.data(), ng1.data(),
        0.05f, 1);
    disldo_backward<SIZE_TYPE, FP4BiPacked, COL_TYPE, RMSpropScalePolicy<float>, false, false>(
        basis.data(), 1, SIZE_TYPE(N), dy.data(), w_block4, dx2.data(), ni2.data(), ng2.data(),
        0.05f, 1);

    for (std::size_t k = 0; k < 2; ++k) {
        CHECK(std::fabs(w_scattered.get_additive_gamma_k(k) - w_block4.get_additive_gamma_k(k)) <
                  1e-4f,
              "additive_gamma_k(%zu) must update identically for block4 vs scattered (got %.6f vs "
              "%.6f)",
              k, w_scattered.get_additive_gamma_k(k), w_block4.get_additive_gamma_k(k));
        for (std::size_t r = 0; r < N; ++r)
            CHECK(std::fabs(w_scattered.get_additive_u_k(r, k) - w_block4.get_additive_u_k(r, k)) <
                      1e-4f,
                  "additive_u_k(row=%zu,k=%zu) must update identically for block4 vs scattered", r,
                  k);
        for (std::size_t c = 0; c < N; ++c)
            CHECK(std::fabs(w_scattered.get_additive_v_k(c, k) - w_block4.get_additive_v_k(c, k)) <
                      1e-4f,
                  "additive_v_k(col=%zu,k=%zu) must update identically for block4 vs scattered", c,
                  k);
    }
}
