...
int main() {
    ...
    static_assert(std::is_same_v<VA::value_type, float>, "value_type should be float (decoded), not uint8_t");
    ...
    CHECK(std::abs(VA::get_w(v, 0) - 1.5f) < 0.05f, "get_w(0) = %f, expected ~1.5", double(VA::get_w(v, 0)));
    CHECK(std::abs(VA::get_imp(v, 0) - (-2.5f)) < 0.05f, "get_imp(0) = %f, expected ~-2.5", double(VA::get_imp(v, 0)));
    CHECK(VA::get_w(v, 1) == 0.0f, "get_w(1) = %f, expected exactly 0", double(VA::get_w(v, 1)));
    CHECK(std::abs(VA::get_imp(v, 1) - 100.0f) < 5.0f, "get_imp(1) = %f, expected ~100", double(VA::get_imp(v, 1)));
    ...
    CHECK(std::abs(sum_w / N - 0.31) < 0.02, "set_stochastic weight mean %f off target 0.31", sum_w / N);
    CHECK(std::abs(sum_imp / N - (-7.4)) < 0.5, "set_stochastic importance mean %f off target -7.4", sum_imp / N);
