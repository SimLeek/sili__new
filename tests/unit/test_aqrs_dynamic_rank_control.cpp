...
static void test_apoptosis_trigger() {
    ...
    for (int i = 0; i < 50; ++i) w.update_scale_gamma_ema_k(0, 0.01f, 0.02f, 0.0f, 0.9f);
    ...
    for (int i = 0; i < 50; ++i) w.update_scale_gamma_ema_k(1, 0.01f, 0.9f, 0.0f, 0.9f);
    ...
    for (int i = 0; i < 50; ++i) w.update_scale_gamma_ema_k(2, 5.0f, 0.01f, 0.0f, 0.9f);
...
static void test_ema_updates_automatically_during_training() {
    ...
    CHECK(w.get_scale_gamma_abs_ema_k(0) == 0.0f,
          "EMA should start at 0 before any training step");
    ...
    std::printf("[ema-integration] gamma final: %.4f, abs EMA final: %.4f\n", gamma_final, ema_final);
    CHECK(ema_final > 0.5f,
          "abs EMA should have tracked gamma's real growth during training, not stayed at 0 (got %.4f, gamma=%.4f)",
    ...
    CHECK(ema_final < gamma_final,
          "EMA of a monotonically-growing signal should lag behind the current raw value (ema=%.4f gamma=%.4f)",
          ema_final, gamma_final);
}
