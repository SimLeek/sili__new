...
int main() {
    ...
    CHECK(oi.size() == 4, "should expose all 4 synapses (2 scattered + 2 block4), got %zu",
          oi.size());

    // row0: cols {1, 5} in ascending order (block4's col1 merged before scattered's col5).
    CHECK(op[0] == 0 && op[1] == 2, "row0 should have 2 entries: ptrs[0]=%d ptrs[1]=%d", (int)op[0],
          (int)op[1]);
    if (op[1] - op[0] == 2) {
        ...
        CHECK(std::abs(oimp[0] - 0.5f) < 1e-4f, "row0 col1 importance should be 0.5, got %.4f",
              oimp[0]);
    }
    ...
    if (op[3] - op[2] == 1) {
        ...
        CHECK(std::abs(ow[op[2]] - 0.5f) < 1e-4f, "row2 col3 weight should be 0.5, got %.4f",
              ow[op[2]]);
    }
    // rows 3-7: nothing.
    ...
    // (the fast-path branch that skips the merge entirely).
    {
        ...
        delta_csr_to_absolute<SIZE_TYPE, FP4BiPacked, COL_TYPE>(w2.connections, op1, oi1, ow1,
                                                                oimp1);
        delta_csr_combined_to_absolute<SIZE_TYPE, FP4BiPacked, COL_TYPE>(w2, op2, oi2, ow2, oimp2);
        CHECK(op1 == op2,
              "empty-block4 fast path: ptrs should match delta_csr_to_absolute exactly");
        CHECK(oi1 == oi2,
              "empty-block4 fast path: indices should match delta_csr_to_absolute exactly");
    }
