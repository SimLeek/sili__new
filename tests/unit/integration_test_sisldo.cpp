...
std::vector<VALUE_TYPE> mse_grad(const std::vector<VALUE_TYPE>& output,
...
template <typename VALUE_TYPE>
VALUE_TYPE mse_loss(const std::vector<VALUE_TYPE>& output, const std::vector<VALUE_TYPE>& desired) {
    VALUE_TYPE loss = 0;
...
TEST_CASE("integration_train_loop", "[integration_train_loop]") {
    ...
    const auto input = make_csr_input<SIZE_TYPE, VALUE_TYPE>(2, 4, {0, 2, 4}, {0, 2, 1, 3},
                                                             {1.0f, 0.5f, 2.0f, 1.5f});
    ...
    auto weights = make_weights<SIZE_TYPE, VALUE_TYPE>(
        n_inputs, n_outputs, {0, 2, 4, 6, 8}, {0, 1, 1, 2, 0, 2, 1, 2},
    ...
    for (int iter = 0; iter < 5; ++iter) {
        ...
        if (iter == 0) {
            ...
            CHECK_VECTOR_ALMOST_EQUAL(
                output, std::vector<VALUE_TYPE>({0.65f, 0.4f, 0.4f, 0.0f, 1.6f, 1.95f}), 1e-4f);
        }
        ...
        auto out_grad_sparse = make_csr_input<SIZE_TYPE, VALUE_TYPE>(
            batch_size, n_outputs, {0, 3, 6}, {0, 1, 2, 0, 1, 2},
            {grad[0], grad[1], grad[2], grad[3], grad[4], grad[5]});

        // output_gradients layout: [out0_b0, out0_b1, out1_b0, out1_b1, out2_b0, out2_b1]
        std::vector<VALUE_TYPE> output_gradients = {grad[0], grad[3], grad[1],
                                                    grad[4], grad[2], grad[5]};
        ...
        sisldo_backward_trivalues(input, weights, out_grad_sparse, input_gradients.data(),
                                  ...
                                  neuron_grad_accum.data(), num_cpus);
...
TEST_CASE("integration_neurogenesis", "[integration_neurogenesis]") {
    ...
    auto input = make_csr_input<SIZE_TYPE, VALUE_TYPE>(
        1, n_inputs, {0, 2}, {1, 3}, {3.0f, 1.0f}); // input 1 dominant, input 3 secondary
    ...
    weights.connections.ptrs[0] =
        std::make_shared<std::vector<SIZE_TYPE>>(std::vector<SIZE_TYPE>(n_inputs + 1, 0));
    ...
    auto out_grad_sparse = make_csr_input<SIZE_TYPE, VALUE_TYPE>(
        1, n_outputs, {0, 2}, {0, 2}, {2.0f, 0.5f}); // output 0 dominant, output 2 secondary
    ...
    genesis_build_probes(weights, neuron_input_accum.data(), neuron_grad_accum.data(), n_inputs,
                         n_outputs, k, num_cpus);
    ...
    CHECK_VECTOR_ALMOST_EQUAL(output2, std::vector<VALUE_TYPE>({3.3f, 0.0f, 2.2f}), 1e-5f);
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        auto og_sparse = make_csr_input<SIZE_TYPE, VALUE_TYPE>(1, n_outputs, {0, 2}, {0, 2},
                                                               {grad3[0], grad3[2]});

        std::vector<VALUE_TYPE> og_dense = {grad3[0], 0.0f, grad3[2]};
        std::vector<VALUE_TYPE> in_grad3(n_inputs, 0.0f);
        sisldo_backward_trivalues(input, weights, og_sparse, in_grad3.data(), og_dense.data(),
                                  neuron_input_accum.data(), neuron_grad_accum.data(), num_cpus);
