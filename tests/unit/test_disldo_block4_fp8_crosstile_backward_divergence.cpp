// Production correctness gate for the fp8 block4 backward cross-tile
// pairing port (disldo_backward.fp8_block4_cross_tile_pairing in
// docs/research/linear_disldo.rst). Mirrors test_disldo_block4_fp32_
// crosstile_scattered_divergence.cpp: identical weights, one arm entirely
...
int main() {
        ...
        for (std::size_t row = 0; row < n_in; ++row) {
            ...
            for (uint32_t bc = 0; bc < block_cols; ++bc) {
                ...
                for (std::size_t col = col_lo; col < col_hi; ++col) {
                    ...
                    // Round-trip through the fp8 grid the same way block4
    ...
    std::printf("fp8 striped scattered-vs-block4: forward err=%.6f dx err=%.6f probe err=%.6f\n",
