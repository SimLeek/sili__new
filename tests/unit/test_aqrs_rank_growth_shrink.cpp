...
static void test_add_multiplier_rank() {
    ...
    for (int i = 0; i < 1000; ++i) step(w, W_STAR, 0.05f);
    std::vector<float> snap_v0(N), snap_o0(N);
    for (std::size_t r = 0; r < N; ++r) snap_v0[r] = w.get_value_scale_k(r, 0);
    for (std::size_t c = 0; c < N; ++c) snap_o0[c] = w.get_output_scale_k(c, 0);
    ...
    for (std::size_t r = 0; r < N; ++r)
        CHECK(std::fabs(w.get_value_scale_k(r, 0) - snap_v0[r]) < 1e-6f,
              "value_scale k=0 row %zu should survive growth unchanged (before=%.6f after=%.6f)",
              r, snap_v0[r], w.get_value_scale_k(r, 0));
    ...
    for (std::size_t r = 0; r < N; ++r)
        CHECK(w.get_value_scale_k(r, 1) == 0.0f, "new value_scale channel k=1 should default to 0.0 (row %zu)", r);
    ...
    Weights w_grown = w;
    for (int i = 0; i < 2000; ++i) step(w_grown, W_STAR, 0.05f);
    ...
    for (int i = 0; i < 3000; ++i) step(w_control, W_STAR, 0.05f);
    ...
    CHECK(mse_after_grow < mse_no_grow - 0.5f,
          "growing to rank-2 should measurably beat staying at rank-1 on a target rank-1 can't fully explain (grown=%.4f control=%.4f)",
          mse_after_grow, mse_no_grow);
}
...
static void test_sub_multiplier_rank() {
    ...
    for (std::size_t r = 0; r < N; ++r) w.set_value_scale_raw_k(r, 1, 0.01f * float(r + 1));
    for (std::size_t c = 0; c < N; ++c) w.set_output_scale_raw_k(c, 1, 0.0f);

    for (int i = 0; i < 3000; ++i) step(w, W_STAR, 0.05f);
    std::vector<float> snap_v0(N), snap_o0(N);
    for (std::size_t r = 0; r < N; ++r) snap_v0[r] = w.get_value_scale_k(r, 0);
    for (std::size_t c = 0; c < N; ++c) snap_o0[c] = w.get_output_scale_k(c, 0);
    ...
    for (std::size_t r = 0; r < N; ++r)
        CHECK(std::fabs(w.get_value_scale_k(r, 0) - snap_v0[r]) < 1e-6f,
              "value_scale k=0 row %zu should survive shrink unchanged (before=%.6f after=%.6f)",
              r, snap_v0[r], w.get_value_scale_k(r, 0));
    for (std::size_t c = 0; c < N; ++c)
        CHECK(std::fabs(w.get_output_scale_k(c, 0) - snap_o0[c]) < 1e-6f,
    ...
    CHECK(shrink_delta > 1e-4f,
          "shrinking a rank-2 layer that trained for real should actually remove a nonzero contribution, not be a no-op (delta=%.6f)",
          shrink_delta);
}
...
static void test_add_addition_rank() {
    ...
    for (int i = 0; i < 1000; ++i) step(w, W_STAR, 0.05f, true);
    std::vector<float> snap_u0(N), snap_v0(N);
    for (std::size_t r = 0; r < N; ++r) snap_u0[r] = w.get_additive_u_k(r, 0);
    for (std::size_t c = 0; c < N; ++c) snap_v0[c] = w.get_additive_v_k(c, 0);
    std::vector<float> y_before_grow = forward_only(w);
    ...
    CHECK(w.additive_rank == 2, "additive_rank should be 2 after growth (got %zu)", w.additive_rank);
    for (std::size_t r = 0; r < N; ++r)
        CHECK(std::fabs(w.get_additive_u_k(r, 0) - snap_u0[r]) < 1e-6f,
              "additive_u k=0 row %zu should survive growth unchanged (before=%.6f after=%.6f)",
              r, snap_u0[r], w.get_additive_u_k(r, 0));
    for (std::size_t c = 0; c < N; ++c)
        CHECK(std::fabs(w.get_additive_v_k(c, 0) - snap_v0[c]) < 1e-6f,
              "additive_v k=0 col %zu should survive growth unchanged (before=%.6f after=%.6f)",
              c, snap_v0[c], w.get_additive_v_k(c, 0));
    ...
    Weights w_grown = w;
    for (int i = 0; i < 2000; ++i) step(w_grown, W_STAR, 0.05f, true);
    ...
    for (int i = 0; i < 3000; ++i) step(w_control, W_STAR, 0.05f, true);
    ...
    std::printf("[add-addition-rank] after grow+train: %.4f, rank-1-only control: %.4f\n",
                mse_after_grow, mse_no_grow);
    CHECK(mse_after_grow < mse_no_grow - 0.5f,
          "growing additive_rank to 2 should measurably beat staying at rank-1 on two independent diagonal spikes (grown=%.4f control=%.4f)",
          mse_after_grow, mse_no_grow);
}
...
static void test_sub_addition_rank() {
    ...
    w.set_additive_u_raw_k(0, 0, 0.01f); w.set_additive_u_raw_k(0, 1, 0.015f);
    w.set_additive_u_raw_k(1, 0, 0.02f); w.set_additive_u_raw_k(1, 1, 0.025f);
    w.set_additive_u_raw_k(2, 0, 0.03f); w.set_additive_u_raw_k(2, 1, 0.035f);

    for (int i = 0; i < 3000; ++i) step(w, W_STAR, 0.05f, true);
    std::vector<float> snap_u0(N), snap_v0(N);
    for (std::size_t r = 0; r < N; ++r) snap_u0[r] = w.get_additive_u_k(r, 0);
    for (std::size_t c = 0; c < N; ++c) snap_v0[c] = w.get_additive_v_k(c, 0);
    std::vector<float> y_before_shrink = forward_only(w);
    ...
    CHECK(w.additive_rank == 1, "additive_rank should be 1 after shrink (got %zu)", w.additive_rank);
    for (std::size_t r = 0; r < N; ++r)
        CHECK(std::fabs(w.get_additive_u_k(r, 0) - snap_u0[r]) < 1e-6f,
              "additive_u k=0 row %zu should survive shrink unchanged (before=%.6f after=%.6f)",
              r, snap_u0[r], w.get_additive_u_k(r, 0));
    for (std::size_t c = 0; c < N; ++c)
        CHECK(std::fabs(w.get_additive_v_k(c, 0) - snap_v0[c]) < 1e-6f,
              "additive_v k=0 col %zu should survive shrink unchanged (before=%.6f after=%.6f)",
              c, snap_v0[c], w.get_additive_v_k(c, 0));
    ...
    CHECK(shrink_delta > 1e-4f,
          "shrinking a rank-2 additive branch that trained for real should actually remove a nonzero contribution, not be a no-op (delta=%.6f)",
          shrink_delta);
}
