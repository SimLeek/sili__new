...
static void run_config(const char* name, std::size_t scale_rank, std::size_t additive_rank,
    ...
    for (std::size_t r = 0; r < n_in; ++r) {
        if (r % 3 == 2)
            continue; // leave every 3rd entry at zero
    ...
    for (std::size_t c = 0; c < n_out; ++c) {
        if (c % 4 == 3)
            continue; // leave every 4th entry at zero
    ...
    disldo_forward<SIZE_TYPE, FP4BiPacked, COL_TYPE>(input.data(), 1, SIZE_TYPE(n_in),
                                                     weights_dense, y_dense.data(), 1);
    sisldo_forward<SIZE_TYPE, FP4BiPacked, COL_TYPE>(in_csr, weights_sparse, y_sparse.data(), 1);
    for (std::size_t c = 0; c < n_out; ++c)
