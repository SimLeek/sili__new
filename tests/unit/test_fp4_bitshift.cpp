...
int main() {
    ...
    for (int code = 0; code < 16; ++code) CHECK(code != 8 || true, "sanity");  // placeholder, real check below
    ...
    for (int i = -8000; i <= 8000; ++i) {
        ...
        CHECK(is_nearest_or_tied(v, code), "encode(%f) = %d (%f) is not nearest (nearest err differs)",
              double(v), int(code), double(FP4_TABLE[code]));
        ++tested;
    }
    ...
    const float boundaries[] = {0.25f,-0.25f,0.75f,-0.75f,1.25f,-1.25f,1.75f,-1.75f,
                                 2.5f,-2.5f,3.5f,-3.5f,5.0f,-5.0f,0.0f,-0.0f,100.0f,-100.0f};
    ...
    CHECK(fp4_encode_bits(NAN) == 0, "encode(NaN) should be code 0, matching the old linear scan's fallback");
