...
// (same design, same several rounds of correction -- see that file's
...
// logical values (col_importance/col_util_dead accumulation is a
// per-cell EMA with no cross-cell reduction, so the two storage formats
// must agree exactly, matching this project's established
// bit-equivalence testing convention for every scattered/block4 pair).
...
static void test_col_importance_accumulates_from_known_values() {
    ...
    auto r =
        apply_amortized_block4_plasticity_step(store, 8, st, cur, /*chunk_size=*/4, /*eta=*/0.0f,
                                               0.99f, 0.95f, 0.5f, 0.1f, 0.0f, 0.0f, 0.5f);
...
static void test_frozen_pool_selects_highest_importance_mature_columns() {
    ...
    apply_amortized_block4_plasticity_step(store, 8, st, cur, 4, 0.0f, 0.99f, 0.95f, 0.5f, 0.1f,
                                           /*reset_fraction=*/0.125f, 0.0f, 0.5f);
...
static void test_dead_pool_excludes_frozen_pool_picks() {
    ...
    for (uint32_t br = 0; br < 2; ++br) {
        for (uint32_t bc = 0; bc < 2; ++bc) {
            ...
            for (uint32_t li = 0; li < 4; ++li) {
                for (uint32_t lj = 0; lj < 4; ++lj) {
                    const uint32_t col = bc * 4 + lj;
                    // col 7: weight=0 (would be the dead-pool minimum too) AND highest importance.
                    h.set_weight(li, lj, col == 7 ? 0.0f : 1.0f);
                    h.set_importance(li, lj, static_cast<float>(col));
                }
            }
        }
    }
    ...
    apply_amortized_block4_plasticity_step(store, 8, st, cur, 4, 0.0f, 0.99f, 0.95f, 0.5f, 0.1f,
                                           0.125f, 0.125f, 0.5f);
    auto r2 = apply_amortized_block4_plasticity_step(store, 8, st, cur, 4, 0.0f, 0.99f, 0.95f, 0.5f,
                                                     0.1f, 0.125f, 0.125f, 0.5f);
    CHECK(st.col_reset_active[7] == 1, "col 7 should be the frozen pick");
    CHECK(st.col_dead_active[7] == 0, "col 7 must NOT also be in the dead pool");
    CHECK(r2.n_dead_this_cycle == 1, "exactly 1 OTHER column should fill the dead pool, got %zu",
          r2.n_dead_this_cycle);
}

static void test_frozen_blend_gated_dead_blend_full_rate_and_unflagged_untouched() {
    Block4Store32 store;
    ...
    st.col_dead_active[4] = 1; // column 4 (tile bc=1, lj=0): ungated
    // column 1 (bc=0, lj=1): neither flag -- must stay byte-identical.
    ...
    CHECK(std::abs(h1.get_importance(0, 0) - 0.8f) < 1e-5f,
          "ungated col 4 importance should shrink by (1-0.2)=0.8, got %f", h1.get_importance(0, 0));
    CHECK(h0.get_weight(0, 1) == 1.0f && h0.get_importance(0, 1) == 1.0f,
          "untouched column (neither pool) must be byte-identical");
}
...
static void test_empty_store() {
    ...
    CHECK(r.n_reset_this_cycle == 0 && r.n_dead_this_cycle == 0,
          "empty store should reset nothing");
}

static void test_bit_equivalence_vs_scattered() {
    // Same logical (row,col)->(weight,importance) values via block4 vs
    // scattered -- col_importance/col_util_dead accumulation must match
    // bit-for-bit (per-cell EMA, no cross-cell reduction -- see the
    // scattered test file's own bit-equivalence precedent).
    Block4Store32 store;
    ...
    for (std::size_t j = 0; j < 4; ++j) {
        ...
        CHECK(st_block4.col_util_dead[j] == st_scattered.col_util_dead[j],
              "bit-equivalence failed on col_util_dead[%zu]: block4=%.9g scattered=%.9g", j,
              double(st_block4.col_util_dead[j]), double(st_scattered.col_util_dead[j]));
    }
}
...
int main() {
    ...
    test_frozen_blend_gated_dead_blend_full_rate_and_unflagged_untouched();
