...
static std::vector<float> naive_attention(const std::vector<float>& Q, const std::vector<float>& K,
                                          const std::vector<float>& V, std::size_t T,
                                          std::size_t d) {
...
static float fd_grad_check(const std::vector<float>& Q, const std::vector<float>& K,
                           const std::vector<float>& V,
                           const std::vector<float>& dO, // upstream gradient
                           std::size_t T, std::size_t d,
                           // analytical grads
                           const std::vector<float>& dQ_anal, const std::vector<float>& dK_anal,
                           const std::vector<float>& dV_anal, float eps = 1e-3f) {
    float max_err = 0.0f;
...
TEST_CASE("sparse_banded_attention: inner_k=0 matches banded_attention exactly",
    ...
    sparse_banded_attention_forward(Q.data(), K.data(), V.data(), out_sparse.data(), T, T, d, 3, 0,
                                    1);
...
TEST_CASE("banded_attention_backward: dV is correct (finite-difference check)",
    ...
    banded_attention_backward(Q.data(), K.data(), V.data(), dO.data(), dQ.data(), dK.data(),
                              dV.data(), T, T, d, T, 1);
...
TEST_CASE("sparse_attention_backward: finite-difference check (full k=T case)",
    ...
    std::vector<float> dQ(T * d, 0), dK(T * d, 0), dV(T * d, 0);
    sparse_attention_backward(Q.data(), K.data(), V.data(), dO.data(), dQ.data(), dK.data(),
                              dV.data(), T, d, T, 1);
...
TEST_CASE("sparse_banded_attention_backward: inner_k=0 backward matches banded_attention_backward",
    ...
    banded_attention_backward(Q.data(), K.data(), V.data(), dO.data(), dQ_b.data(), dK_b.data(),
                              dV_b.data(), T, T, d, bw, 1);
    sparse_banded_attention_backward(Q.data(), K.data(), V.data(), dO.data(), dQ_s.data(),
                                     dK_s.data(), dV_s.data(), T, T, d, bw, 0, 1);
...
static std::vector<float> naive_causal_attention(const std::vector<float>& Q,
...
TEST_CASE("gaussian_attention: sigma->0 with center on an integer key concentrates output on that "
          "key's V",
          "[attention][gaussian]") {
    ...
    gaussian_attention_forward(Q.data(), Kmat.data(), V.data(), out.data(), 1, K, d, centers.data(),
                               sigmas.data(), 1, false);
...
TEST_CASE("gaussian_attention: with sigma very large, reduces to plain dense softmax attention "
          "(Gaussian bias vanishes)",
          "[attention][gaussian]") {
    const std::size_t T = 4, d = 4;
    std::mt19937 rng(41);
    std::normal_distribution<float> dist(0.0f, 1.0f);
    ...
    std::vector<float> out(T * d, 0.0f);
    gaussian_attention_forward(Q.data(), K.data(), V.data(), out.data(), T, T, d, centers.data(),
                               sigmas.data(), 1, false);
...
TEST_CASE("gaussian_attention_backward: dCenters and dSigmas match finite differences",
    ...
    gaussian_attention_backward(Q.data(), Kmat.data(), V.data(), dO.data(), dQ.data(), dK.data(),
                                dV.data(), dC.data(), dS.data(), T, K, d, centers.data(),
                                sigmas.data(), 1, false);

    auto fwd_loss = [&](const std::vector<float>& c, const std::vector<float>& s) {
        std::vector<float> o(T * d, 0.0f);
        gaussian_attention_forward(Q.data(), Kmat.data(), V.data(), o.data(), T, K, d, c.data(),
                                   s.data(), 1, false);
        float loss = 0.0f;
        for (std::size_t j = 0; j < T * d; ++j)
            loss += o[j] * dO[j];
