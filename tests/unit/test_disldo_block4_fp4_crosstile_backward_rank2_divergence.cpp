// Rank-2 correctness gate for the fp4 block4 backward cross-tile pairing
// generalization (process_tile_pair_fp4 is now rank-N generic, not just
// rank==1 -- see disldo_backward.fp4_block4_cross_tile_pairing in
// docs/research/linear_disldo.rst). Same striped scattered-vs-block4
// pattern as test_disldo_block4_fp4_crosstile_backward_divergence.cpp, but
...
int main() {
    ...
    // Same rank-2 value_scale_k(row,1)/output_scale_k(col,1) values applied
    // identically to both arms, so the k=1 channel isn't a no-op.
    ...
    // Post-backward forward probe (also checks the k=1 value_scale/
    // output_scale RMSprop update itself matched between arms).
    ...
    std::printf("fp4 rank2 striped scattered-vs-block4: forward err=%.6f dx err=%.6f probe "
